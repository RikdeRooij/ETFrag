...
#define MAX_HUD_BGS 6

// bacon
typedef struct hud_s {
	int healthbar[4];		// x, y, width, height
	int staminabar[4];		// x, y, width, height
	int chargebar[4];		// x, y, width, height
	int weaponicon[4];		// x, y, width, height
	int ammo[3];			// x, y, scale
	int compass[3];			// x, y, size
	int hp[3];			// x, y, scale
	int xp[3];			// x, y, scale
	int xpadd[3];			// x, y, scale
	int draws[1];			// y
	int lagometer[2];		// x, y
	int skilltexts[4];		// x, y, numoff, plment
	int skillboxes[4];		// x, y, width, height
	int fireteam[3];		// x, y, width
	int head[4];			// x, y, width, height
	int flagcov[2];			// x, y
	int popuptext[2];		// x, y
	int chattext[3];		// x, y, WIDTH (was scale)
	int votefttext[2];		// x, y
	int livesleft[3];		// x, y, scale
	int centerprint[1];		// y
	int killspree[3]; // x, y, size
	int backgrounds[MAX_HUD_BGS][4];		// x, y, width, height
} hud_t;
...
typedef struct {
	...
	HudrectDef_t skilltexts;
	HudrectDef_t skillboxes;
	HudrectDef_t skillbars;
	HudrectDef_t fireteam;
	HudrectDef_t head;
	HudrectDef_t flagcov;
	HudrectDef_t popuptext;
	HudrectDef_t chattext;
	HudrectDef_t votefttext;
	HudrectDef_t livesleft;
	HudrectDef_t centerprint;

	HudrectDef_t killspree;

	HudrectDef_t backgrounds[MAX_HUD_BGS];
} hudRect_t;
...
// JPW FIXME NOTE: max_weap_banks > max_weap_banks_mp so this should be OK, but if that changes, change this too
	...
	int killspreePrintTime;
	char killspreePrint[1024];
...
//----(SA)	cursor hints
	...
	qhandle_t	SquareAutomapMaskShader;
	qhandle_t	compass3;
...
//	skill_testicon
	...
	qhandle_t		transWhite;
	qhandle_t		transRed;
	qhandle_t		transGreen;
	qhandle_t		transBlue;

	qhandle_t		gsfloortest;
	qhandle_t		gsskytest;
	...
	qhandle_t	compassColorTest_oran;
	qhandle_t	compassColorTest_blue;
...
//	int				notifyLastPos;
	...
	int eventHandling; // codePRK FIX boolean? enum cgameEvent_e, int?
...
#ifdef GS_CCHARACTERS
	int limbo_custombody[TEAM_NUM_TEAMS][NUM_PLAYER_CLASSES]; // 0 to 8
#endif
...
extern vmCvar_t cg_drawObjectiveFP;
extern vmCvar_t cg_drawFireteamIcons;
extern vmCvar_t cg_drawPickupMsg;
extern vmCvar_t cg_popupDelay;
extern vmCvar_t cg_shadowedPopups;
extern vmCvar_t cg_numPopups;
extern vmCvar_t cg_hudColor;
extern vmCvar_t cg_hudAlpha;
extern vmCvar_t cg_drawHardcore;

extern vmCvar_t cg_visWeapSelectTime;


extern vmCvar_t cg_testVar;


extern vmCvar_t cg_x;


extern vmCvar_t			cg_hitSounds;

// codePRK adminpanel
extern vmCvar_t	ui_adminLevel;
extern vmCvar_t	ui_adminFlags;
...
void CG_AddKillSpreeItemBig( const char *str ); // codePRK
