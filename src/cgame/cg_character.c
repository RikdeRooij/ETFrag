...
qboolean CG_RegisterCharacter( const char *characterFile, bg_character_t *character )
{
	...
	if( !(character->skin = trap_R_RegisterSkin( filename )) ) {
		...
		char *customSkinname;
		for( i=0; i<MAX_CUSTOMSKINS; i++ ) {
			customSkinname = va( "%s_c%i_%s.skin", buf, i, characterDef.skin );
			character->customSkins[i] = 0;
			//if( !(character->customSkins[i] = trap_R_RegisterSkin( customSkinname )) ) {
			character->customSkins[i] = trap_R_RegisterSkin( customSkinname );
			//if( !(character->customSkins[i]) ) {
			//	CG_Printf( S_COLOR_YELLOW "WARNING: failed to register CUSTOM skin '%s'\n", customSkinname );
			//}
			if( (character->customSkins[i]) ) {
				CG_Printf( S_COLOR_GREEN "found custom skin: '%s'\n", customSkinname );
			}
		}
		//character->customSkins[0] = trap_R_RegisterSkin( "models/players/temperate/allied/soldier/body_c0_soldier.skin" );
#endif
...
bg_character_t *CG_CharacterForClientinfo( clientInfo_t *ci, centity_t *cent )
{
	...
	if( cent && cent->currentState.eType == ET_CORPSE ) {
		// codePRK invalid as it was always -1
