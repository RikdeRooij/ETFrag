...
void CG_RailTrail2_Ext( vec3_t start, vec3_t end, vec3_t color, int lifeTime ) 
{
    localEntity_t	*le;
    refEntity_t		*re;
...
void CG_RailTrail_Ext( vec3_t start, vec3_t end, qboolean box, vec3_t color, int lifeTime ) 
{
    vec3_t	diff, v1, v2, v3, v4, v5, v6;

    if (!CG_AllocLocalEntity())
        return;

    // use cg_railTrailTime cvar
    if ( lifeTime < 0 )
        lifeTime = cg_railTrailTime.integer;


    if(!box) {	// just a line
        CG_RailTrail2_Ext( start, end, color, lifeTime );
        return;
    }
    ...
    v2[1] -= diff[1];
    v3[2] -= diff[2];
    CG_RailTrail2_Ext( start, v1, color, lifeTime);
    CG_RailTrail2_Ext( start, v2, color, lifeTime);
    CG_RailTrail2_Ext( start, v3, color, lifeTime);
    ...
    v5[1] += diff[1];
    v6[2] += diff[2];
    CG_RailTrail2_Ext( end, v4, color, lifeTime);
    CG_RailTrail2_Ext( end, v5, color, lifeTime);
    CG_RailTrail2_Ext( end, v6, color, lifeTime);

    CG_RailTrail2_Ext( v2, v6, color, lifeTime);
    CG_RailTrail2_Ext( v6, v1, color, lifeTime);
    CG_RailTrail2_Ext( v1, v5, color, lifeTime);

    CG_RailTrail2_Ext( v2, v4, color, lifeTime);
    CG_RailTrail2_Ext( v4, v3, color, lifeTime);
    CG_RailTrail2_Ext( v3, v5, color, lifeTime);
}
...
// ------------------------
                // codePRK pickupModel
                if( !weaponInfo->weaponModel[W_PU_MODEL].model && viewType == W_TP_MODEL ) {
...
// -------------------------------
                // codePRK weaponicon
                if( !weaponInfo->weaponIcon[0] && weaponInfo->weaponIcon[1] ) { 
                    int len, len2, newlen;
                    int j;
                    char newfn[64] = "";
                    char select[64];
                    ...
                    //select = "_select";
                    Q_strncpyz( select, "_select", sizeof(select) );
gunstargoto:
                    len = strlen( filename );
                    len2 = strlen( select );
                    ...
                    // hmmm, we should check if it ends with _select
                    for( i = newlen; i < len; i++ ) {
                        j = ( i - newlen ); // start 0
                        selcheck[j] = filename[i];
                    }

                    if( Q_stricmp( selcheck, select) ) {
                        //CG_Printf( "^3[GUNSTAR] ^1'%s'^7 != ^1'%s'\n", selcheck, select );

                        doit = qfalse;
                        // tryagain
                        if( !Q_stricmp( select, "_select") ) {
                            //CG_Printf( "^3[GUNSTAR] trying again for '%s'\n", filename );
                            Q_strncpyz( select, "_select.tga", sizeof(select) );
                            goto gunstargoto;
                        }
                    } else {
                        doit = qtrue;
                    ...
                    if( doit ) {
                        //CG_Printf( "^5[GUNSTAR] FIL: %s\n", filename );
                        for( i = 0; i < newlen; i++ ) {
                            newfn[i] = filename[i];
                        }

                        //CG_Printf( va("^2[GUNSTAR] REG: %s\n", newfn) );
                        // how can I check for warnings?
                        test = trap_R_RegisterShader( newfn );
                        if( test ) {
                            weaponInfo->weaponIcon[0] = test;
                        }
                    }
                }
// -------------------------------
...
#ifdef GS_TURRETS
        case WP_TURRET:				filename = "turret.weap"; break;
#endif
...
// ------------------------------------------------------------------------------------------
// codePRK objective pickup

void CG_DrawObjItem( playerState_t *ps ) {
    refEntity_t	hand;
    vec3_t		angles;
    vec3_t		gunoff;
    float		fovOffset;

    if ( !cg_drawObjectiveFP.integer ) {
        return;
    }

    if(!( cgs.weapons & WPF_WEAP_OBJ )) {
        return;
    }
    ...
    if( ps->weapon == WP_FG42SCOPE || ps->weapon == WP_GARAND_SCOPE || ps->weapon == WP_K43_SCOPE ) {
        return;
    }
    ...
    if( ps && !cg.renderingThirdPerson ) {
        ...
        angles[PITCH] -= (10.f); // lil bit towards player
        angles[ROLL] += (20.f); // roll bit right
    ...
    // hand.hModel = trap_R_RegisterModel( "models/multiplayer/secretdocs/secretdocs.md3" );
    // ent.hModel = cgs.gameModels[ es->modelindex2 ];

    hand.hModel = cg.ObjItem;

    // DEBUG print
    //CG_Printf( va("^1model0: %d\n", (item - bg_itemlist) ) );
    
    if (!hand.hModel) {
        //hand.hModel = cgs.media.thirdPersonBinocModel; // debugs
        //CG_Printf("returning due to: !hand.hModel\n");
        return;
    }
...
========================
*/
static void CG_AddWeaponWithPowerups( refEntity_t *gun, int powerups, playerState_t *ps, centity_t *cent, qboolean shadowtest ) {
    // add powerup effects
...
void CG_AddPlayerWeapon( refEntity_t *parent, playerState_t *ps, centity_t *cent, qboolean shadowtest ) {
    ...
    if( drawpart ) {
        ...
        if( isPlayer && !cg.renderingThirdPerson && cg_drawGun.integer >= 2) {
            switch(cg_drawGun.integer) {
                ...
                case 3:
                    gun.customShader = cgs.media.transRed;
                    break;

                case 4:
                    gun.customShader = cgs.media.transGreen;
                    break;

                case 5:
                    gun.customShader = cgs.media.transBlue;
                    break;
                ...
                default:
                    //gun.customShader = cgs.media.transWhite;
                    break;

            }
        }

        CG_AddWeaponWithPowerups( &gun, cent->currentState.powerups, ps, cent, shadowtest );
    }
                ...
                if( drawpart ) {
                    ...
                    if( isPlayer && !cg.renderingThirdPerson && cg_drawGun.integer >= 2) {
                        switch(cg_drawGun.integer) {
                            ...
                            case 3:
                                barrel.customShader = cgs.media.transRed;
                                break;

                            case 4:
                                barrel.customShader = cgs.media.transGreen;
                                break;

                            case 5:
                                barrel.customShader = cgs.media.transBlue;
                                break;
                            ...
                            default:
                                //barrel.customShader = cgs.media.transWhite;
                                break;

                        }
                    }

                    CG_AddWeaponWithPowerups( &barrel, cent->currentState.powerups, ps, cent, shadowtest );
                    ...
                    if( weaponNum == WP_SATCHEL_DET && i == W_PART_1 ) {
                        ...
                        CG_AddWeaponWithPowerups( &satchelDetPart, cent->currentState.powerups, ps, cent, shadowtest );
                        ...
                        if( ps && !cg.renderingThirdPerson && cg.predictedPlayerState.weaponstate != WEAPON_RAISING ) {
                            ...
                            CG_AddWeaponWithPowerups( &bipodLeg, cent->currentState.powerups, ps, cent, shadowtest );
    ...
    if( ps->eFlags & EF_MOUNTEDTANK ) {
        ...
        if( cg_drawGun.integer >= 2) {
            switch(cg_drawGun.integer) {
                ...
                case 3:
                    hand.customShader = cgs.media.transRed;
                    break;

                case 4:
                    hand.customShader = cgs.media.transGreen;
                    break;

                case 5:
                    hand.customShader = cgs.media.transBlue;
                    break;
                ...
                default:
                    //hand.customShader = cgs.media.transWhite;
                    break;
        ...
        CG_AddWeaponWithPowerups( &hand, cg.predictedPlayerEntity.currentState.powerups, ps, &cg.predictedPlayerEntity, qfalse );
...
void CG_MG42EFX (centity_t *cent)
{
    ...
    for ( num = 0 ; num < cg.snap->numEntities ; num++ ) {
        ...
        if( mg42->currentState.eType == ET_MG42_BARREL &&
            ...
            if( mg42->currentState.weapon == WP_MOBILE_MG42 ) { // codePRK turret muzzle
                VectorMA( point, 28, forward, point );
                point[2] += 6.0f;
            }
...
//		int			num;
        vec3_t		forward;
            ...
            if( cent->currentState.eType == ET_MG42_BARREL )
            {
                ...
                if( cent->currentState.weapon == WP_MOBILE_MG42 ) { // codePRK turret muzzle
                    VectorMA( muzzle, 28, forward, muzzle );
                    muzzle[2] += 6.0f;
                }
