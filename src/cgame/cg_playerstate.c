...
qboolean CG_WeaponIsExplosive( int weapon )
{
	switch( weapon ) {
		case WP_GRENADE_LAUNCHER:
		case WP_GRENADE_PINEAPPLE:
...
void CG_CheckLocalSounds( playerState_t *ps, playerState_t *ops ) {
	...
	if ( ops->persistant[PERS_HITS] != ps->persistant[PERS_HITS] && cg_hitSounds.integer > 0 ) {
		
		if( !CG_WeaponIsExplosive( ps->weapon ) ) { // like in etpro
			// 1: body,head
			// 2: headshots only
			// 3: body,head, no team
			// 4: headshots, no team
			...
			// maybe make this bitflags
			if ( ps->persistant[PERS_HITS] < ops->persistant[PERS_HITS] ) {
				if ( cg_hitSounds.integer <= 2 || cg_hitSounds.integer == 5 )
				...
				if ( cg_hitSounds.integer <= 4 )
					trap_S_StartSound (NULL, ps->clientNum, CHAN_AUTO, cgs.media.headShot );
			} else {
				if ( cg_hitSounds.integer == 1 || cg_hitSounds.integer == 3 ) // BODYSHOT: 1 or 3
