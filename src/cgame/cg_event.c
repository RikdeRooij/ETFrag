...
// ----------------------
	...
	if (( 1 ) && message) {
		...
		// you killed yourself. (no attacker)
		CG_AddPMItem2( PM_OBIT, targetName, icon, " ", scale, OB_YELLOW );
...
// ----------------------
	if (message) {
		...
		if ( (cg_obituary.integer % 2) == 1 ) {
			...
			CG_AddPMItem( PM_DEATH, finalMsg, deathShader, OB_YELLOW );
		}
		return;
	}
	...
	if( attacker == cg.snap->ps.clientNum ) {
		...
		if ( ci->team == ca->team ) {
			...
			s = va("%s %s", CG_TranslateString( "You fragged" ), targetName );
		}
...
// ------------------------------
		// codePRK graphic obituary
		if (( 1 ) && message && message2) {
			int scale=1;

			if ( weapon > WP_NONE && weapon < WP_NUM_WEAPONS && !overrideIcon ){
				//if ( cg_drawSmallPopupIcons.integer && cg_weapons[ weapon ].weaponIcon[0] ){
				//	icon = cg_weapons[ weapon ].weaponIcon[0];
				//	scale = weaponIconScale(weapon);
				//} else
			...
			CG_AddPMItem2( PM_OBIT, attackerName, icon, targetName, scale, color);
		...
		if (message) {
			...
			if ( message2 ) {
				...
				if ( (cg_obituary.integer % 2) == 1 ) {
					...
					CG_AddPMItem( PM_DEATH, finalMsg, deathShader, color );
				}
//				CG_Printf( "%s %s %s%s\n", targetName, message, attackerName, message2 );
			}
			return;
...
================
*/
static void CG_ItemPickup( int itemNum, qboolean newweap ) {
	int itemid;
	...
	if( cg_drawPickupMsg.integer ) {
		CG_AddPMItem( PM_MESSAGE, va( "Picked up %s", CG_PickupItemText( itemNum ) ), cgs.media.pmImages[PM_MESSAGE], color );
	}
...
//----(SA)	do things based on the players head being above/below the water brushes top surface. (since the waves can potentially be /way/ above/below that)
	...
	case EV_ITEM_PICKUP_QUIET:
		DEBUGNAME("EV_ITEM_PICKUP");
		{
			...
			if ( es->number == cg.snap->ps.clientNum || ( es->clientNum == cg.snap->ps.clientNum ) ) {
				CG_ItemPickup( index, es->otherEntityNum2 ? qtrue : qfalse );
			}
...
// jpw
	...
	case EV_WEAPONSWITCHED:
		...
		if( (es->clientNum == cg.snap->ps.clientNum) && es->eventParm == 56 ) {
			//CG_Printf( "^5CG_OutOfAmmoChange\n" );
	...
	case EV_CHANGE_WEAPON:
		...
		//Add here the carry feature (maybe set the Alt Weapons off?)
		if ( es->number == cg.snap->ps.clientNum ) {
			cg.carryWeaponTime = cg.time + cg_visWeapSelectTime.integer;
		}
		break;
...
/*	case EV_FLAMEBARREL_BOUNCE:
	...
	case EV_RAILTRAIL:
		if( es->dmgFlags >= 0 ) {
			...
			color[0] = es->eventParms[0];
			color[1] = es->eventParms[1];
			color[2] = es->eventParms[2];
			lifetime = es->effect1Time;
			if( lifetime < 0 )
				lifetime = cg_railTrailTime.integer;

			clientnum = es->otherEntityNum2;
			
			if( (clientnum >= 0) && (clientnum == cg.snap->ps.clientNum) ) {
				CG_RailTrail_Ext( es->origin2, es->pos.trBase, 0, color, lifetime );
			}
			else if( clientnum == -1 ) {
				CG_RailTrail_Ext( es->origin2, es->pos.trBase, 0, color, lifetime );
			}
	...
	case EV_POPUPMESSAGE:
		{
			...
			if( str ) {
				CG_AddPMItem( cent->currentState.effect1Time, str, shader, color );
			}
