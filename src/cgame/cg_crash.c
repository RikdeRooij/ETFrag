...
void CrashHandler(int signal, siginfo_t *siginfo, ucontext_t *ctx) {
	...
	if(segvloop < 2) {
		...
		crashLog=fopen(va("%s/%s/client_crash.log",homepath,gamepath),"w");

		Crash_Printf("---------- Crash Information ----------\n");
		//Crash_Printf("   Please forward to etpub mod team.   \n");
		Crash_Printf("---------------------------------------\n");
		Crash_Printf("Version: %s %s Linux\n", GAMEVERSION, MOD_VERSION);
		Crash_Printf("Map: %s\n",cgs.rawmapname);
...
LONG CALLBACK win32_exception_handler(LPEXCEPTION_POINTERS e) {
	...
	//Crash_Printf("   Please forward to etpub mod team.   \n");
	Crash_Printf("---------------------------------------\n");
	Crash_Printf("Version: %s %s Win32\n", GAMEVERSION, MOD_VERSION);
	Crash_Printf("Map: %s\n",cgs.rawmapname);
