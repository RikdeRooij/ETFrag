...
*/
#ifdef GS_BREAKLEGS
	if( cg.snap->ps.eFlags & EF_LEGSBROKEN ) { 
		cg.zoomSensitivity = 0.5;
	} else
#endif
...
/*
		for( i=0; i<3; i++ ) {
			if( i == YAW ) {
				// 180 | 0 | -180 (180 flips to -180)
				if( ( ps->viewangles[YAW] < -90 && cg.refdefViewAngles[YAW] > 90 ) || 
					( ps->viewangles[YAW] > 90 && cg.refdefViewAngles[YAW] < -90 ) ) {
						// flip direction cos it would be shorter
						flip = qtrue;
				}
			}
			if( (cg.refdefViewAngles[i] >= ps->viewangles[i] - margin) && (cg.refdefViewAngles[i] <= ps->viewangles[i] + margin) ) {
				cg.refdefViewAngles[i] = ps->viewangles[i];
			} else {
				float anglediff;
				
				anglediff = abs(ps->viewangles[i] - cg.refdefViewAngles[i]);

				speed = 2.f;
				if( anglediff > 45.f ) speed = anglediff;
				...
				if( cg.refdefViewAngles[i] > ps->viewangles[i] )
					cg.refdefViewAngles[i] -= speed;
				if( cg.refdefViewAngles[i] < ps->viewangles[i] )
					cg.refdefViewAngles[i] += speed;

				if( i == YAW ) {
					if ( cg.refdefViewAngles[i] > 180.0 )
						cg.refdefViewAngles[i] = -180.0;
					if ( cg.refdefViewAngles[i] < -180.0 )
						cg.refdefViewAngles[i] = 180.0;
				}
			}
...
void CG_DrawActiveFrame( int serverTime, stereoFrame_t stereoView, qboolean demoPlayback ) {
	...
	// codePRK fill the nothing with a color
	CG_FillRect( 0, 0, SCREEN_WIDTH, SCREEN_HEIGHT, colorClr );
// -----------------------------------------------
	...
	if(cg.mvCurrentMainview != NULL && cg.snap->ps.pm_type != PM_INTERMISSION) {
		...
		if( !cg.showGameView && !cgs.dbShowing ) {
			if( !cg.snap->ps.persistant[PERS_HWEAPON_USE] ) {
				...
				CG_DrawObjItem( &cg.predictedPlayerState );
			} else {
	...
	if( cg.riflehack ) { // && !cg.showGameView

		CG_DrawGrenadelauncherTrace();

		if( cg.latestMissile ) // && !cg.showGameView
		{
			rectDef_t rect;
			//vec4_t	color = {1, 1, 1, .8};
			//vec4_t	colorShade = {0, 0, 0, .8};
...
#ifdef GS_DEBUG
	if( cg.snap->ps.eFlags & EF_LEGSBROKEN )
		CG_Text_Paint_Ext( 16, 16, 0.25f, 0.25f, colorRed, va( "EF_LEGSBROKEN" ), 0, 0, 0, &cgs.media.limboFont2 );
