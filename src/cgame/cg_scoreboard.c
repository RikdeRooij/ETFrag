...
// JPW NERVE -- mission time & reinforce time
		...
		if ( cgs.gametype == GT_WOLF_STOPWATCH ) {
			...
			CG_Text_Paint_Ext( x + 300 - w*0.5f, y + 13, 0.25f, 0.25f, 
						tclr, s, 0, 0, 0, &cgs.media.limboFont1 );
		}
...
static void WM_DrawClientScore( int x, int y, score_t *score, float *color, float fade, qboolean show_lives ) {
	...
	if( cg_scoreboard.integer == SCOREBOARD_KD ) {
		maxchars -= 3;
	}
	...
	if( drawScore ) {
		if( cg_scoreboard.integer == SCOREBOARD_KD ) {
			CG_DrawSmallString( tempx, y, 
				//va( "%i/%i", score->kills, score->deaths ), fade );
				//va( "%.3f", ((float)score->kills / (float)score->deaths) ), fade );
				va( "%3i", score->kills ), fade );
...
int QDECL CG_SortScoresByKD( const void *a, const void *b ) {
	score_t *score_a = &cg.scores[*((int*)a)];
	score_t *score_b = &cg.scores[*((int*)b)];

	if(score_a->kills > score_b->kills)
		return -1;
	else if(score_a->kills < score_b->kills)
		return 1;
	// kills are equal
	else if(score_a->deaths > score_b->deaths)
		return -1;
	else if(score_a->deaths < score_b->deaths)
		return 1;
...
static int WM_TeamScoreboard( int x, int y, team_t team, float fade, int maxrows, int absmaxrows ) {
	...
	switch(cg_scoreboard.integer) {
		case SCOREBOARD_KD:
				Q_strncpyz(scorename, "Kills", sizeof(scorename));
			break;
		case SCOREBOARD_KR:
			if(cgs.killRating)
...
//	height = SMALLCHAR_HEIGHT * maxrows;
	...
	for(i=0; i<cg.numScores; i++)
		sort[i] = i;
	if(cg_scoreboard.integer == SCOREBOARD_KD) {
		qsort(sort, cg.numScores, sizeof(int), CG_SortScoresByKD);
	}
	else if(cg_scoreboard.integer == SCOREBOARD_KR) {
