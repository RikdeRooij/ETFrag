...
//	if( *v ) {
//		newInfo.latchCustomBody = atoi(v);
//	}
#endif
...
void CG_SetLerpFrameAnimationRate( centity_t *cent, clientInfo_t *ci, lerpFrame_t *lf, int newAnimation ) {
	...
	int				transitionMin = -1, oldAnimNum; // oldAnimTime
...
static void CG_PlayerSprites( centity_t *cent ) {
	...
	if( cg_drawFireteamIcons.integer )
	{
		fireteamData_t* ft;
		if( (ft = CG_IsOnFireteam( cent->currentState.number )) != NULL ) { // this person is on a fireteam
			//if( ft == CG_IsOnFireteam( cg.clientNum ) && cgs.clientinfo[ cent->currentState.number ].selected ) {
			if( ft == CG_IsOnFireteam( cg.clientNum ) ) { // this person is on same fireteam as you (client)
...
// CGS_	  (gunstar)
void CGS_PositionRotatedEntityOnTagOffset( refEntity_t *entity, const refEntity_t *parent, const char *tagName, vec3_t *offset ) {
	int				i;
...
//	float			gumsflappin = 0;	// talking amplitude
	...
	if( cent->currentState.eFlags & EF_MOUNTEDTANK ) {
		...
		for ( num = 0 ; num < cg.snap->numEntities ; num++ ) {
			...
			if( (mg42->currentState.eType == ET_MG42_BARREL || mg42->currentState.eType == ET_AAGUN) &&
				...
				//if( mg42->currentState.weapon == WP_MOBILE_MG42 ) { // codePRK turret
				//	VectorMA ( mg42->currentState.pos.trBase, -24, forward, playerOrigin );
				//} else {
	...
	if( cent->currentState.eType == ET_CORPSE && cent->currentState.time2 == 1 ) {
		...
		if( (ci->customBody > 0) && (ci->customBody <= MAX_CUSTOMSKINS ) ) {
			if( character->customSkins[ci->customBody-1] ) {
				body.customSkin	= character->customSkins[ci->customBody-1];
			} else {
...
// DEBUG
	...
	// codePRK secondary when it's not in use
	if( 1 ) {
		vec3_t mat[3];
		weaponInfo_t	*weapon;
		vec3_t orig;
		...
		if( (cent->currentState.weapon != ci->secondaryweapon) &&
			(cent->currentState.weapon != GET_ALT_WEAPON(ci->secondaryweapon)) &&
			(IS_VALID_WEAPON(ci->secondaryweapon))
			) {

			weapon = &cg_weapons[ci->secondaryweapon];

			acc.hModel = weapon->weaponModel[W_PU_MODEL].model;
			if( ( ci->team == TEAM_AXIS ) &&
				weapon->weaponModel[W_PU_MODEL].skin[TEAM_AXIS] )
				acc.customSkin = weapon->weaponModel[W_PU_MODEL].skin[TEAM_AXIS];
			else if( ( ci->team == TEAM_ALLIES ) &&
					 weapon->weaponModel[W_PU_MODEL].skin[TEAM_ALLIES] )
				acc.customSkin = weapon->weaponModel[W_PU_MODEL].skin[TEAM_ALLIES];
			else
				acc.customSkin = weapon->weaponModel[W_PU_MODEL].skin[0];


memcpy( mat, acc.axis, sizeof(mat));

	//AxisToAngles( acc.axis, angles );
	VectorClear( angles );
	angles[PITCH] = 90;
	angles[ROLL] = 140;
	angles[YAW] = 140;
	...
	VectorSet( orig, 0.f, -2.f, 0.f ); // to the right

			// CG_PositionEntityOnTag( &acc,	&body,	"tag_bright", 0, &orig);
			CGS_PositionRotatedEntityOnTagOffset( &acc,	&body,	"tag_bright", &orig );
			CG_AddRefEntityWithPowerups( &acc, cent->currentState.powerups, ci->team, &cent->currentState, cent->fireRiseDir );

memcpy( acc.axis, mat, sizeof(acc.axis));
		}

		if( (cent->currentState.weapon != ci->weapon) &&
			(cent->currentState.weapon != GET_ALT_WEAPON(ci->weapon)) &&
			(IS_VALID_WEAPON(ci->weapon))
			) {

			weapon = &cg_weapons[ci->weapon];
...
memcpy( mat, acc.axis, sizeof(mat));
	//AxisToAngles( acc.axis, angles );
	VectorClear( angles );
	//if( cent->lerpAngles[PITCH] < 0 ) // LOOKING UP
	//	angles[PITCH] += (cent->lerpAngles[PITCH] / 3.f);
	//else 
	//	angles[PITCH] += (cent->lerpAngles[PITCH] / 4.f);
	angles[PITCH] += 90;
	angles[ROLL] += 270; // rotation
	AnglesToAxis( angles, acc.axis );

	VectorSet( orig, 0.f, 2.f, -2.f ); // forward, left, up

	if( cent->lerpAngles[PITCH] < 0 ) { // LOOKING UP
		orig[2] += (cent->lerpAngles[PITCH] / 32.f);
		orig[0] -= (cent->lerpAngles[PITCH] / 7.f);
	} else {
		orig[2] += (cent->lerpAngles[PITCH] / 32.f); // more is less up
		orig[0] -= (cent->lerpAngles[PITCH] / 7.f); // more is less back
	}

	AngleVectors( angles, NULL, right, NULL );
	VectorMA( orig, (cent->lerpAngles[PITCH] / 10.f), right, orig );

			//CG_PositionEntityOnTag( &acc,	&body,	"tag_back", 0, &orig);
			CGS_PositionRotatedEntityOnTagOffset( &acc,	&body,	"tag_back", &orig );
			CG_AddRefEntityWithPowerups( &acc, cent->currentState.powerups, ci->team, &cent->currentState, cent->fireRiseDir );
...
static void CG_PlayerAnimation_IDLE( bg_character_t* character, refEntity_t *body ) {
	int animIndex;

	// <animation name>		<first frame>	<length>
	// relaxed_idle_2h_1	898				195
	// relaxed_idle_1h_1	1093			1
	// relaxed_idle_no		1094			219
//		nazi_salute			4243			29
//		alert_idle_2h		1633			60
//		alert_idle_1h		1704			70

	// legs alert_crchidle_2h torso stand_machinegun

	//stand_machinegun		2253	35

	//animIndex = BG_AnimScriptAnimation( clientNum, character->animModelInfo, ANIM_MT_IDLE, qtrue );

	//animIndex = 1094 + ((cg.time/50) % 219);
	animIndex = 2253 + ((cg.time/50) % 35);

	body->frame = body->oldframe = body->torsoFrame = body->oldTorsoFrame = animIndex;
	body->frameModel = body->oldframeModel = body->torsoFrameModel= body->oldTorsoFrameModel = character->animModelInfo->animations[0]->mdxFile;
}

void CG_DrawPlayer_Limbo_GS( float x, float y, float w, float h, int clientNum, int team, int cls, int weaponNum ) {
	vec3_t			origin;
	vec3_t			angles;
	refdef_t		refdef;
	int				distance;
	vec4_t			hcolor = { 0.0, 0.0, 0.0, 0.2 };
	bg_character_t* character;
	// -------------------------------
	//centity_t		*cent;
	clientInfo_t	*ci;
	...
	playerOrigin[2] = 24.2f; // makes feet on 0 point
	...
	refdef.fov_x = 12.0f; //w/35.0f;
	refdef.fov_y = 12.0f; //h/35.0f;
...
// #####################################################################################
	...
	if(1) {
		...
		VectorCopy( vec3_origin, floororg );
		floororg[0] -= size;

		for( i=0; i<4; i++ ) {
			for( j=0; j<4; j++ ) {
				verts[i].modulate[j] = 192;
			}
		}

		// top
		VectorCopy( floororg, verts[0].xyz );
		verts[0].xyz[0] -= depth;
		verts[0].xyz[1] -= size;
		verts[0].st[0] = 0;
		verts[0].st[1] = 0;
		VectorCopy( floororg, verts[1].xyz );
		verts[1].xyz[0] -= depth;
		verts[1].xyz[1] += size;
		verts[1].st[0] = 0;
		verts[1].st[1] = 1;
		VectorCopy( floororg, verts[2].xyz );
		verts[2].xyz[0] += depth;
		verts[2].xyz[1] += size;
		verts[2].st[0] = 1;
		verts[2].st[1] = 1;
		VectorCopy( floororg, verts[3].xyz );
		verts[3].xyz[0] += depth;
		verts[3].xyz[1] -= size;
		verts[3].st[0] = 1;
		verts[3].st[1] = 0;

		trap_R_AddPolyToScene( cgs.media.gsfloortest, 4, verts );
		...
		verts[0].xyz[0] = -depth;
		verts[0].xyz[1] = -size;
		verts[0].xyz[2] = (size + 40); //120 + 60;
		verts[0].st[0] = 0;
		verts[0].st[1] = 0;

		verts[1].xyz[0] = -depth;
		verts[1].xyz[1] = size;
		verts[1].xyz[2] = (size + 40); //120 + 60;
		verts[1].st[0] = 0;
		verts[1].st[1] = 1;

		verts[2].xyz[0] = -depth;
		verts[2].xyz[1] = size;
		verts[2].xyz[2] = (-size + 40); //-40 - 60;
		verts[2].st[0] = 1;
		verts[2].st[1] = 1;

		verts[3].xyz[0] = -depth;
		verts[3].xyz[1] = -size;
		verts[3].xyz[2] = (-size + 40); //-40 - 60;
		verts[3].st[0] = 1;
		verts[3].st[1] = 0;

		trap_R_AddPolyToScene( cgs.media.gsskytest, 4, verts );
	}
...
// #####################################################################################
	...
	if( team != TEAM_SPECTATOR ) {
		...
		//CG_RenderPlayerModelToRefdef( cent );

		memset( &rbody, 0, sizeof(rbody) );
		memset( &rhead, 0, sizeof(rhead) );
		memset( &racc, 0, sizeof(racc) );
		//cent = &cg_entities[clientNum];
		//CG_PlayerAngles( cent, rbody.axis, rbody.torsoAxis, rhead.axis );
		AnglesToAxis( vec3_origin, rbody.axis );
		AnglesToAxis( vec3_origin, rbody.torsoAxis );
		AnglesToAxis( vec3_origin, rhead.axis );
		AxisCopy( rbody.torsoAxis, racc.axis );
		//CG_AnimPlayerConditions( character, cent );
		CG_PlayerAnimation_IDLE( character, &rbody );
		...
		rbody.hilightIntensity = 0.2f;
		rhead.hilightIntensity = 0.2f;
		racc.hilightIntensity = 0.2f;

		// ========================================
		// rbody
		{
			int	custombody = ((cgs.limbo_custombody[team][cls]) - 1);

			//CG_Printf("^2customBody: %i\n", (custombody)); 

			if( custombody >= 0 && custombody < MAX_CUSTOMSKINS ) {
				//rbody.hModel		= BG_GetCharacter( team, custombody )->mesh;
				//rbody.customSkin	= BG_GetCharacter( team, custombody )->skin;
				rbody.hModel		= character->mesh;
				rbody.customSkin	= character->customSkins[custombody];
			} else {
				rbody.hModel		= character->mesh;
				rbody.customSkin	= character->skin;
			}
		}


		VectorCopy( playerOrigin, rbody.origin );
		VectorCopy( lightorigin, rbody.lightingOrigin );
		rbody.renderfx = renderfx;
		VectorCopy( rbody.origin, rbody.oldorigin );

		//rbody.customShader = cgs.media.realshadows_playerTEX;
		trap_R_AddRefEntityToScene( &rbody );

		// ========================================
		// rhead
		if( !(rhead.hModel = character->hudhead) ) {
			return;
		}
		rhead.customSkin = character->hudheadskin;
		VectorCopy( lightorigin, rhead.lightingOrigin );
		CG_PositionRotatedEntityOnTag( &rhead, &rbody, "tag_head" );
		rhead.renderfx = renderfx;

		//CG_HudHeadAnimation( character, &cent->pe.head, &rhead.oldframe, &rhead.frame, &rhead.backlerp, HD_IDLE1 );

		//rhead.customShader = cgs.media.realshadows_playerTEX;
		trap_R_AddRefEntityToScene( &rhead );
		...
		//if( !(cent->currentState.eFlags & EF_DEAD) ) 
		//{
		//	CG_AddPlayerWeapon( &rbody, cent, cent, qtrue );
		//}
		if(1) {
			refEntity_t	gun;
			refEntity_t	barrel;
			...
			CG_PositionEntityOnTag( &gun, &rbody, "tag_weapon", 0, NULL);
			trap_R_AddRefEntityToScene( &gun );
			...
			for( i = W_PART_1; i < W_MAX_PARTS; i++ ) {
				...
				if( barrel.hModel ) {
					CG_PositionEntityOnTag( &barrel, &rbody, weapon->partModels[W_FP_MODEL][i].tagName, 0, NULL );
					...
					//CG_AddWeaponWithPowerups( &barrel, NULL, NULL, NULL, NULL );
		...
		// ========================================
		// racc
		VectorCopy( lightorigin, racc.lightingOrigin );
		racc.renderfx = renderfx;

		for( i = ACC_BELT_LEFT; i < ACC_MAX; i++ ) {
			if( !(character->accModels[i]) )
				continue;
			racc.hModel = character->accModels[i];
			racc.customSkin = character->accSkins[i];

			// Gordon: looted corpses dont have any accsserories, evil looters :E
			switch(i)
			{
				case ACC_BELT_LEFT:
					CG_PositionEntityOnTag( &racc,	&rbody,	"tag_bright", 0, NULL);
					break;
				case ACC_BELT_RIGHT:
					CG_PositionEntityOnTag( &racc,	&rbody,	"tag_bleft", 0, NULL);
					break;

				case ACC_BELT:
					CG_PositionEntityOnTag( &racc,	&rbody,	"tag_ubelt", 0, NULL);
					break;
				case ACC_BACK:
					CG_PositionEntityOnTag( &racc,	&rbody,	"tag_back", 0, NULL);
					break;
				...
				case ACC_MOUTH3:		// hat3					
					...
					if( i == ACC_RANK ) {
						...
						racc.customShader = rankicons[ ci->rank ][ 1 ].shader;
					}

					CG_PositionEntityOnTag( &racc,	&rhead,	"tag_mouth", 0, NULL);
					break;
				
				// weapon and weapon2
				// these are used by characters who have permanent weapons attached to their character in the skin
				case ACC_WEAPON:	// weap
					CG_PositionEntityOnTag( &racc,	&rbody,	"tag_weapon", 0, NULL);
					break;
				case ACC_WEAPON2:	// weap2
					CG_PositionEntityOnTag( &racc,	&rbody,	"tag_weapon2", 0, NULL);
					break;
			...
			//racc.customShader = cgs.media.realshadows_playerTEX;

			trap_R_AddRefEntityToScene( &racc );
		}

	} // team_spec

	VectorCopy( playerOrigin, lightorigin );
	lightorigin[0] += 64; // to-screen
	lightorigin[1] -= 64; // up

	trap_R_AddLightToScene( lightorigin, 512, 0.9f, 0.95f, 1.0f, 1.0f, 0, 0 );
