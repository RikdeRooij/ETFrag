...
static void CG_StatsDown_f(void)
{
	if(!cg.demoPlayback) {
		...
		// codePRK stats
		// if on team, draw own stats, always, also in limbo
		// i = (cg.mvTotalClients > 0) ? (cg.mvCurrentActive->mvInfo & MV_PID) : cg.snap->ps.clientNum;
		if ( cg_stats.integer ) {
			i = ((cg.snap->ps.teamNum == TEAM_SPECTATOR) && (cg.mvTotalClients > 0)) ? (cg.mvCurrentActive->mvInfo & MV_PID) : cg.clientNum;
		}
...
static void CG_LoadHud_f( void ) {
	...
	if ( trap_Argc() < 2 ) {
		...
		CG_Printf("^3usage: /LoadHud [hudname]. Type ^7/loadhud ? ^3to show a list of available huds.\n");
	} else {
		...
		} else if( cg_hud.string == filename ) { // it's already this name
			CG_UpdateHud(); // changing cvar would have eventually call this
		} else {
			trap_Cvar_Set("cg_hud", filename);
		}
...
static void CG_CPM_f( void ) {
	...
	CG_AddPMItem( PM_MESSAGE, CG_Argv(1), cgs.media.voiceChatShader, color );
}
...
#ifdef GS_DEBUG
static void CG_KS_f( void ) {
	CG_AddKillSpreeItemBig( "^1KILLINGSPREE! \n^7code^zPRK^7: ^35 kills in a row!" );


}


static void CG_PKILL_f( void ) {
	vec3_t color =	{ 1.f, 1.f, 1.f };
	//icon = cg_weapons[ weapon ].weaponIcon[1];
	qhandle_t icon =	cg_weapons[WP_THOMPSON].weaponIcon[0];
	
	if( !icon ) {
		icon = cg_weapons[WP_THOMPSON].weaponIcon[1];
	}

	CG_AddPMItem2(PM_OBIT, "^1Attacker", icon, "^3Target", 2.0f, color);
}

static void CG_PKILL2_f( void ) {
	vec3_t color =	{ 1.f, 1.f, 1.f };
	//icon = cg_weapons[ weapon ].weaponIcon[1];
	qhandle_t icon =	cg_weapons[WP_COLT].weaponIcon[0];

	if( !icon ) {
		icon = cg_weapons[WP_COLT].weaponIcon[1];
	}

	CG_AddPMItem2(PM_OBIT, "^1Attacker", icon, "^3Target", 1.0f, color);
}

static void CG_CTEST_f( void ) {
	trap_SendConsoleCommand( va("ui_adminsysCPM \"%s\"\n", CG_Argv(1)) );

	CG_PriorityCenterPrint( "You fragged ^2Nobody ^7:o", CP_DEFAULTHEIGHT, BIGCHAR_WIDTH * 0.6, 0 );
}
#endif

#ifdef GS_UUID
#include "get_id.h"
static void CG_CINFO_f( void ) {
	char *UUID;
	char *MACadress;
	char *otherstuff;
	...
	otherstuff = GetMserial();
	CG_Printf( "^2serial: ^7\'%s\'\n", otherstuff );

	otherstuff = GetMmodel();
	CG_Printf( "^2model:  ^7\'%s\'\n", otherstuff );

	UUID = GetUUID_Encrypt();
	CG_Printf( "^1UUID: ^7\'%s\'\n", UUID );
...
// codePRK ui
void CG_AdminPanel_f(void) {

	CG_EventHandling( CGAME_EVENT_NONE, qfalse );
	trap_UI_Popup( UIMENU_ADMIN );
	//trap_SendConsoleCommand( "vstr cmd;");
}
...
void CG_InitConsoleCommands( void ) {
	...
	trap_AddCommand("etfrag_version");
