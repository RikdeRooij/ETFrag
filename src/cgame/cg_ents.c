...
static void CG_General( centity_t *cent ) {
	...
	if( cent->currentState.eType == ET_MG42_BARREL )
	{
		...
		// codePRK turret
		if( cent->currentState.weapon == WP_MOBILE_MG42 ) {
			float testScale = (cent->currentState.effect3Time / 100.f);
			VectorScale( ent.axis[0], testScale, ent.axis[0] );
			VectorScale( ent.axis[1], testScale, ent.axis[1] );
			VectorScale( ent.axis[2], testScale*0.9, ent.axis[2] );
...
#ifdef GS_TURRETS
	if( cent->currentState.powerups == STATE_CANTBUILDHERE ) {
		ent.customShader = cgs.media.transRed;
	}
#endif
...
//----(SA)	end
	...
	if(es->modelindex2) {	// modelindex2 was specified for the ent, meaning it probably has an alternate model (as opposed to the one in the itemlist)
		...
		//if( item->classname == "team_CTF_blueflag" || item->classname == "team_CTF_redflag" ) {
		//if( item->pickup_name == "Blue Flag" || item->pickup_name == "Red Flag" || item->pickup_name == "Objective" ) {
		if ( 1 ) {
			gitem_t *obji;

			obji = BG_FindItem("Blue Flag");
			if (!obji)
				obji = BG_FindItem("Red Flag"); // prob never happens tho
			if (!obji)
				obji = BG_FindItem("Objective");

			// codePRK FIXME: this doesn't seem right...
			es->modelindex = (obji - bg_itemlist);
			if ( es->modelindex ) {
				cg.ObjItem = cgs.gameModels[ es->modelindex2 ];
			}
		}
		// ---------------------------------------------------------
	} else {
...
// -------------------------------------------
	...
	if( cg.riflehack ) {
		if ( s1->clientNum == cg.snap->ps.clientNum ) { // cg.clientNum
...
qboolean CG_AddLinkedEntity( centity_t *cent, qboolean ignoreframe, int atTime ) {
	...
	if (!ignoreframe && (cent->processedFrame == cg.clientFrame) && cg.mvTotalClients < 2 && !cg.subscene ) {
...
qboolean CG_AddEntityToTag( centity_t *cent ) {
	...
	if (cent->processedFrame == cg.clientFrame && cg.mvTotalClients < 2 && !cg.subscene	) {
		// already processed this frame
		return qtrue;
	}
