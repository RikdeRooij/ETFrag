...
int CG_AlignedX( int x, int width, int maxwidth, int hudeditWidth, qboolean center ) {
	int w, w2;
	int result;

	if( x >= 0 ) {

		if( x < HUD_ALIGN ) { // align left
			w = 0;
			w2 = 0;
		} else if( center ) { // if allow centered
			if( x > (640 - hudeditWidth - HUD_ALIGN ) ) { // align right if on far right
				w = width;
				w2 = maxwidth;
			} else { // center if not right (or left)
				w = width / 2;
				w2 = maxwidth / 2;
			}
		} else { // if not on far left and not allowing centered -> align right
			w = width;
			w2 = maxwidth;
		}
...
static void CG_DrawTeamInfo( void ) {
	...
	float 	scaleVal;
	qhandle_t flag = 0;
	int realchatloc_x; // = CHATLOC_X;
	int realchatloc_y; // = CHATLOC_Y;
	...
	realchatloc_x = cg.hud.chattext[0];
	realchatloc_y = cg.hud.chattext[1];
	chatWidth = cg.hud.chattext[2];

	scaleVal = 0.2f; //cg.hud.chattext[2]/100.f;
	...
	if( cgs.teamLastChatPos != cgs.teamChatPos ) {
		...
		for( i = cgs.teamChatPos - 1; i >= cgs.teamLastChatPos; i-- ) {
			...
			trap_R_SetColor( hcolor );
			CG_DrawPic( realchatloc_x, 
				realchatloc_y - (cgs.teamChatPos - i)*lineHeight,
			...
			CG_Text_Paint_Ext( realchatloc_x +
...
static void CG_DrawBannerPrint( void ) {
	...
	if ( !cg.bannerPrintTime ) {
		if( cgs.hardcore && cg_drawHardcore.integer ) {
			y = CG_Text_Height_Ext("A", cg_fontBPScale.value, 0, &cgs.media.font4) * 1.5;
			w = CG_Text_Width_Ext("HARDCORE MODE", 0.2f, 0, &cgs.media.limboFont1);
			x = ( SCREEN_WIDTH - w ) / 2;
			CG_Text_Paint_Ext( x, y, 0.2f, 0.2f, colorRed, "HARDCORE MODE", 0, 0, ITEM_TEXTSTYLE_SHADOWED, &cgs.media.limboFont1 );
		}
		return;
	}
...
// codePRK killingspree draw

void CG_AddKillSpreeItemBig( const char *str ) {
	char buff[MAX_STRING_CHARS];
	int		i, len, textlen;
//	qboolean neednewline = qfalse;


	Q_strncpyz( cg.killspreePrint, str, sizeof(cg.killspreePrint) );

	// turn spaces into newlines, if we've run over the linewidth
	len = strlen( cg.killspreePrint );
	for ( i = 0, textlen = 0; i < len; i++, textlen++) {
		
		// "\n" in center/banner prints are seen as new lines.
		// kw: this is also done serverside in etpub
		if ( cg.killspreePrint[i] == '\\' && cg.killspreePrint[i+1] == 'n' ) {
			Q_strncpyz( buff, &cg.killspreePrint[i+2], sizeof(buff));
			cg.killspreePrint[i] = '\n';
			cg.killspreePrint[i+1] = 0;
			Q_strcat(cg.killspreePrint, sizeof(cg.killspreePrint), buff);
		}
		if( cg.killspreePrint[i] == '\n')
			textlen = 0;

		if( Q_IsColorString( &cg.killspreePrint[i] ) )
			textlen -= 2;
	}

	// post-editing to print text correctly into the console
	for(i=0,len=0; i<strlen(cg.killspreePrint); i++ ) {
		// replace newlines with spaces
		if(cg.killspreePrint[i] == '\n' ) {
			if(len != 0 && buff[len-1] != ' ') {
				buff[len] = ' ';
		...
		// no spaces at the beginning of the string
		if(len == 0 && cg.killspreePrint[i] == ' ') 
			continue;
		buff[len] = cg.killspreePrint[i];
		len++;
	}
	buff[len] = 0;
...
// codePRK centerprint
static void CG_DrawSpecialCenterString( int y, float scale, char *text, int duration ) // , vec4_t color
{
// CG_DrawSpecialCenterString( x, y, cg_fontCPScale.value, color, va( "^%c%s", colorchar, linebuffer) );
	int starttime = cg.centerPrintTime - 2000;
	int fadetime = 1000;
	int waittime = duration + 2000 - fadetime; //1000 * cg_centertime.value;
	vec4_t	color = { 1.f, 1.f, 1.f, 0.f };
	vec4_t	effcolor = { 1.f, 1.f, 1.f, 0.f };
	float fade;
	float textscale;
	float effscale;
	int x;
...
// TEXT
	// fade out
	color[3] = ((float)((starttime + waittime + fadetime) - cg.time) / fadetime);
	//color[3] = 1.0;

	if( color[3] > 1.f ) { color[3] = 1.f; }
	if( color[3] < 0.f ) { color[3] = 0.f; return; }


	// fade in and out
	fade = (float)((starttime + 500) - cg.time)/500;
	effcolor[3] = ((sin( M_PI * fade ))/3);
...
// TEXT
	...
	w = CG_Text_Width_Ext( text, textscale, 0, &cgs.media.limboFont1 );
	x = ( SCREEN_WIDTH - w ) / 2;
	CG_Text_Paint_Ext( x, y, textscale, textscale , color, text, 0, 0, ITEM_TEXTSTYLE_SHADOWED, &cgs.media.limboFont2 );
...
// EFFECT
	...
	if( (starttime + 500) >= cg.time ) {
		w = CG_Text_Width_Ext( text, effscale, 0, &cgs.media.limboFont1 );
		x = ( SCREEN_WIDTH - w ) / 2;
		CG_Text_Paint_Ext( x, y, effscale, effscale, effcolor, text, 0, 0, ITEM_TEXTSTYLE_SHADOWED, &cgs.media.limboFont2 );
	}
}
...
static void CG_DrawCenterString( void ) {
	...
	while ( 1 ) {
		...
		// codePRK centerprint
		if ( !Q_stricmpn( linebuffer, "You killed", 10 )
			|| !Q_stricmpn( linebuffer, "You fragged", 11 )
			|| !Q_stricmpn( linebuffer, "^1FIGHT!", 8 )
				) {
			float tscale = cg_fontCPScale.value;

			if( !Q_stricmpn( linebuffer, "^1FIGHT!", 8 ) )
				tscale *= 1.4;


			CG_DrawSpecialCenterString( y, tscale, va( "^%c%s", colorchar, linebuffer), 1000 * cg_centertime.value );
		} else {
...
void CG_DrawCrosshairReal( void ) {
	float		w, h;
	qhandle_t	hShader;
	float		f;
	float		x, y;
...
//		VectorCopy( cg.snap->ps.origin, start );
	...
	if(cg.snap->ps.viewangles[PITCH] > 30 && 
		(cg.snap->ps.eFlags & EF_CROUCHING)) {
		start[2] += 30;
	}
	else {
	...
	CG_Trace( &tr, start, NULL, NULL, end, cg.clientNum, MASK_SOLID );
	CG_RailTrail( NULL, start, end, 0 );
	VectorCopy( tr.endpos, end );

	dist = VectorDistance( start, end );
	if( dist < 750.f ) dist = 750.f; if( dist > 1500.f ) dist = 1500.f;
	scale = 1500.f / dist * .5f; // min: 0.5  max: 1
...
static void CG_DrawCrosshair(void) {
	...
	if( cg_testVar.integer > 0 ) {
		CG_DrawCrosshairReal();
		return;
	}
	...
	if ( cg.crosshairClientNum > MAX_CLIENTS ) {
		...
		if( cg_entities[cg.crosshairClientNum].currentState.powerups & 
			...
			//     spotted even from great distance.
				...
				cgs.realism // cant scan with crosshair
				) {
...
//	float alpha = cg_specAlpha.value;
	...
	*/
	...
	w = CG_DrawStrlen( s );
	CG_DrawStringExt( 320 - w * cw/2, 440, s, colorWhite,  // Elf
...
static void CG_DrawVote(void) {
	...
	if( cgs.voteTime ) {
		...
		if(!Q_stricmp(str3, "\\hidevote")) {
			sec = ( 2000 - ( cg.time - cgs.voteTime ) ) / 1000;
		}
...
static void CG_DrawWarmup( void ) {
	...
	if(!sec) {
		if((cgs.gamestate == GS_WARMUP && !cg.warmup) || cgs.gamestate == GS_WAITING_FOR_PLAYERS) {
			...
			w = CG_DrawStrlen( s1 );
			CG_DrawStringExt(320 - w * cw/2, y, s1, colorYellow, qfalse, qtrue, cw, cw * 3 / 2, 0);
			...
			if(!cg.demoPlayback && cg.snap->ps.persistant[PERS_TEAM] != TEAM_SPECTATOR &&
			  (!(cg.snap->ps.pm_flags & PMF_FOLLOW) || (cg.snap->ps.pm_flags & PMF_LIMBO))) {
				...
				w = CG_DrawStrlen( s2 );
				CG_DrawStringExt(320 - w * cws/2, y + cws + padding*2, s2, colorWhite, qfalse, qtrue, cws, (int)(cws * 1.5), 0);
			}
...
*/
	...
	w = CG_DrawStrlen( s );
	CG_DrawStringExt( 320 - w * cw / 2, y, s, colorYellow, qfalse, qtrue, cw, cw * 3 / 2, 0 );
	...
	if ( cgs.gametype == GT_WOLF_STOPWATCH ) {
		...
		w = CG_DrawStrlen( s );
		CG_DrawStringExt( 320 - w * cws/2, y + cw + padding*2, s, colorWhite, 
			qfalse, qtrue, cws, cws * 3 / 2, 0 );

		w = CG_DrawStrlen( s1 );
		CG_DrawStringExt( 320 - w * cws/2, y + cw + padding*2 + cws + padding, s1, colorWhite, 
			qfalse, qtrue, cws, cws * 3 / 2, 0 );

		w = CG_DrawStrlen( s2 );
		CG_DrawStringExt( 320 - w * cws/2, y + cw + padding*2 + cws*2 + padding, s2, colorWhite, 
			qfalse, qtrue, cws, cws * 3 / 2, 0 );
	}
}
...
static void CG_DrawKillSpreePrint( void ) {
	char	*start;
	int		l;
	int		x, y, w;
	float	*color;
	...
	//font = CG_GetFont( F_MISC ); // HUDCHARS
	//size = ((CG_GetFontSize()/2)*2.5);
	//size2 = ((CG_GetFontSize()/2)*2.5);

	//if( !font ) {
		font = &cgs.media.limboFont1;
	//}

	if ( !cg.killspreePrintTime ) {
		return;
	}
	...
	color = CG_FadeColor( cg.killspreePrintTime, KS_TIME );
	if ( !color ) {
		cg.killspreePrintTime = 0;
		return;
	}
	trap_R_SetColor( color );

	start = cg.killspreePrint;
	...
	y = cg.hud.killspree[1] - 4;

	size = cg.hud.killspree[2]/100.f;
	size2 = cg.hud.killspree[2]/100.f;

	charHeight = CG_Text_Height_Ext("A", size,
				0, &cgs.media.limboFont1);

	fulls = 200; // CG_Text_Width_Ext("XXXXXXXXXXXXXXXX: 999 kills in a row!"
	...
	while ( 1 ) {
		...
		for ( l = 0; l < strlen( cg.killspreePrint ); l++ ) {
			if ( !start[l] || start[l] == '\n' ) {
				break;
			}
			if( Q_IsColorString( &start[l] ) )
		...
		//w = MINICHAR_WIDTH * CG_DrawStrlen( linebuffer );
		w = CG_Text_Width_Ext(linebuffer, size,
				0, font);

		//x = ( SCREEN_WIDTH - w ) / 2; // center
		//x = 6; // left


		x = CG_AlignedX( cg.hud.killspree[0], w, fulls, fulls, qtrue);
		...
		//size = 0.2f; // newlines get smaller
		// lets take a precentage
		//size = ((size2 / 25.f) * 20.f);
		size = (size2 * 0.8f); // 80 %
...
//	else
		...
		//if( cg_drawCompass.integer ) { // codePRK square compass
			x -= 32;
			y -= 32;
			w += 64;
			h += 64;
...
/*	basex = 520 - 16;
	...
	//CG_DrawPic( basex + 4, basey + 4, basew - 8, baseh - 8, cgs.media.SquareAutomapMaskShader );
...
static void CG_DrawPlayerHealthBar( rectDef_t *rect ) {
	...
	if (rect->h >= rect->w) {
		...
		flags = 16|64;
		bgcolour[0] = bgcolour[1] = bgcolour[2] = 0.f; // BLACK
		  colour[0] = 1.f;   colour[1] = 0.1f;   colour[2] = 0.1f; // RED
	}
	...
	if (rect->h >= rect->w) {
		...
		// make place for icon, and be a lil smaller in height
		CG_FilledBar( rect->x + rect->h + 1, rect->y + ((rect->h*0.33f)/2), rect->w - (rect->h + 1), rect->h - (rect->h*0.33f), colour, NULL, bgcolour, frac, flags );
	}

	trap_R_SetColor( NULL );
	// * althud 1
	if (rect->h >= rect->w) { // not for the althud
		...
		CG_DrawPic( rect->x, rect->y, rect->h, rect->h, cgs.media.hudHealthIcon );
	}
}
...
static void CG_DrawStaminaBar( rectDef_t *rect ) {
	...
	//int flags = 1|4|16|64;
	// * althud horizontal/vertical bars
	// just based on size for the moment
	if (rect->h >= rect->w) {
		flags = 1|4|16|64;
	} else { //(rect->h < rect->w)
		flags = 16|64;
		bgcolour[0] = 0.f; bgcolour[1] = 0.f; bgcolour[2] = 0.f; //bgcolour[3] = 1.f; // want it black!
	}
	...
	if (rect->h >= rect->w) {
		...
		// make place for icon, and be a lil smaller in height
		CG_FilledBar( rect->x + rect->h + 1, rect->y + ((rect->h*0.33f)/2), rect->w - (rect->h + 1), rect->h - (rect->h*0.33f), color, NULL, bgcolour, frac, flags );
	}
	...
	if (rect->h >= rect->w) { // not for the althud
		...
		CG_DrawPic( rect->x, rect->y, rect->h, rect->h, cgs.media.hudSprintIcon );
	}
}
...
static void CG_DrawWeapRecharge( rectDef_t *rect ) {
	...
	if (rect->h >= rect->w) {
		...
		flags = 16|64; // added 64
		bgcolor[0] = 0.f; bgcolor[1] = 0.f; bgcolor[2] = 0.f; //bgcolor[3] = 1.f; // want it black!
	}
...
//	}
	...
	if (rect->h >= rect->w) {
		...
		// make place for icon, and be a lil smaller in height
		CG_FilledBar( rect->x + rect->h + 1, rect->y + ((rect->h*0.33f)/2), rect->w - (rect->h + 1), rect->h - (rect->h*0.33f), color, NULL, bgcolor, barFrac, flags );
	}
	...
	if (rect->h >= rect->w) { // not for the althud
		...
		// smaller in width
		CG_DrawPic( rect->x + rect->h*0.25f, rect->y, rect->h*0.5f, rect->h, cgs.media.hudPowerIcon );
	}
}
...
//	vec4_t			colorFaded = { 1.f, 1.f, 1.f, 0.3f };
	...
	if(cg.hud.weaponicon[0] >= 0) {
		...
		rect.x = cg.hud.weaponicon[0];
		rect.y = cg.hud.weaponicon[1];
		rect.w = cg.hud.weaponicon[2];
		rect.h = cg.hud.weaponicon[3];
...
if( !cgs.hardcore ) {
	...
	if(cg.hud.ammo[0] >= 0) {
		...
		if( value3 >= 0 ) {
			...
			CG_Text_Paint_Ext( cg.hud.ammo[0] - CG_Text_Width_Ext( buffer, ammocountScale, 0, &cgs.media.limboFont1 ), cg.hud.ammo[1], ammocountScale, ammocountScale, colorWhite, buffer, 0, 0, ITEM_TEXTSTYLE_SHADOWED, &cgs.media.limboFont1 );
...
static void CG_DrawSkillBar( float x, float y, float w, float h, int skill ) {
	int i;
	float blockheight = ( h - 4 ) / (float)(NUM_SKILL_LEVELS - 1);
	float blockwidth = ( w - (NUM_SKILL_LEVELS - 1) ) / (float)(NUM_SKILL_LEVELS - 1);
	float draw_y;
	float draw_x;
	vec4_t colour;
	float x1, y1, w1, h1;
	...
	//		CG_DrawPic( x, draw_y, w, blockheight, cgs.media.hudBorderVert2 );
		...
		for( i = 0; i < NUM_SKILL_LEVELS - 1; i++ ) {
			...
			CG_FillRect( draw_x, y, blockwidth, h, colour );
			...
			CG_DrawRect_FixedBorder( draw_x, y, blockwidth, h, 1, colorBlack );
	//		CG_DrawPic( x, draw_y, w, blockheight, cgs.media.hudBorderVert2 );
			draw_x += ( blockwidth + 0.5f ); // + 1
		}
	}
}
...
static void CG_DrawPlayerStats( void ) {
	...
	//if(cg.hud.hp[0] >= 0) {
	if( (cg.hud.hp[0] >= 0) && (!cgs.hardcore) ) {
		int ax = cg.hud.hp[0];
		int maxwidth;
		int hudeditwQ;
// XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
// codePRK health text color
		vec4_t hpcolor = { 1.f, 1.f, 1.f, 1.f };
		static int hp;
		static int hpChangeTime;
		static qboolean morecolor;

		if( hp < cg.snap->ps.stats[STAT_HEALTH] ) {
			hpChangeTime = cg.time;
			if( hp <= (cg.snap->ps.stats[STAT_HEALTH] - 20) ) {
				morecolor = qtrue;
			} else {
				morecolor = qfalse;
			}
		} else if( hp > cg.snap->ps.stats[STAT_HEALTH] ) {
			hpChangeTime = -cg.time;	
			if( hp >= (cg.snap->ps.stats[STAT_HEALTH] + 20) ) {
				morecolor = qtrue;
			} else {
		...
		if( hpChangeTime + 500 > cg.time ) {
			if( morecolor ) {
				hpcolor[0] = hpcolor[2] = 0.0f;
			} else {
				hpcolor[0] = hpcolor[2] = .6f;
			}
		} else if( hpChangeTime - 500 < -cg.time ) {
			if( morecolor ) {
				hpcolor[1] = hpcolor[2] = 0.0f;
			} else {
				hpcolor[1] = hpcolor[2] = .6f;
			}
		}
// XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
		...
		w = CG_Text_Width_Ext( str, scaleVal, 0, &cgs.media.limboFont1 );

		maxwidth = CG_Text_Width_Ext( "100", scaleVal, 0, &cgs.media.limboFont1 );
		hudeditwQ = maxwidth + (CG_Text_Width_Ext( "HP", scaleVal * 0.8f, 0, &cgs.media.limboFont1 ) + 2);
		ax = CG_AlignedX( cg.hud.hp[0], w, maxwidth, hudeditwQ, qtrue);

		CG_Text_Paint_Ext( ax, cg.hud.hp[1], scaleVal, scaleVal, hpcolor, str, 0, 0, ITEM_TEXTSTYLE_SHADOWED, &cgs.media.limboFont1 );
		CG_Text_Paint_Ext( ax + w + 2, cg.hud.hp[1], scaleVal * 0.8f, scaleVal * 0.8f, hpcolor, "HP", 0, 0, ITEM_TEXTSTYLE_SHADOWED, &cgs.media.limboFont1 );
	}
...
// -----------------------------------------------------------------------------
	...
	for( i = 0; i < 3; i++ ) {
		...
		if(cg.hud.skillboxes[0] >= 0) {
			if(cg.hud.skillboxes[3] >= cg.hud.skillboxes[2]) { // more height than width
				float w = (cg.hud.skillboxes[2] / 3);
				float h = (cg.hud.skillboxes[3] / 4);
				CG_DrawSkillBar( (i * (w)) + cg.hud.skillboxes[0] + 1, cg.hud.skillboxes[1], w-2, cg.hud.skillboxes[3], ci->skill[skill] );
				
				// vertical
				//if(cg.hud.skillpics[i][0] < 0) { // auto attach it if we want bars but no skillpics, saves the math :D
					CG_DrawPic( (i * (w)) + cg.hud.skillboxes[0], cg.hud.skillboxes[1] + h*4 + 1, w, h, cgs.media.skillPics[skill] );
				//}
		
			} else {
				float w = (cg.hud.skillboxes[2] / 4);
				float h = (cg.hud.skillboxes[3] / 3);
				CG_DrawSkillBar( cg.hud.skillboxes[0], (i * (h)) + cg.hud.skillboxes[1], cg.hud.skillboxes[2], h-2, ci->skill[skill] );

				// horizontal
				//if(cg.hud.skillpics[i][0] < 0) {
					CG_DrawPic( cg.hud.skillboxes[0] - w - 2, (i * h) + cg.hud.skillboxes[1] - 1, w, h, cgs.media.skillPics[skill] );
				//}
			}
		}
		if(cg.hud.skilltexts[0] >= 0) {
			...
			//x = cg.hud.skilltexts[0];
			//y = cg.hud.skilltexts[1];
			numoff = cg.hud.skilltexts[2]; // pic+num or num under pic
			plment = cg.hud.skilltexts[3]; // vertical or horizontal

			//scaleVal = cg.hud.skilltexts[2]/100.f;
			scaleVal = 0.25f;
			// 0.25 = * 56
			// 0.20 = * 70
			...
			if( numoff ) { // put number under pic
				...
				//if(cg.hud.skillpics[i][0] < 0) {
				CG_DrawPic( cg.hud.skilltexts[0] + (i * h/2), cg.hud.skilltexts[1] + (i * v), size, size, cgs.media.skillPics[skill] );
				//}

				CG_Text_Paint_Ext(cg.hud.skilltexts[0] + (i * h/2) + (size/4),cg.hud.skilltexts[1] + (i * v) - (v ? 4 : 0) + size + size,
				...
				//if(cg.hud.skillpics[i][0] < 0) {
				CG_DrawPic( cg.hud.skilltexts[0] + (i * h), cg.hud.skilltexts[1] + (i * v), size, size, cgs.media.skillPics[skill] );
				//}

				CG_Text_Paint_Ext(cg.hud.skilltexts[0] + (i * h) + (scaleVal * 56) + (size/4),cg.hud.skilltexts[1] + (i * v) + size - (h ? 2 : 0),
					scaleVal, scaleVal, colorWhite, 
					va("%d",ci->skill[skill]),
					0, 0, ITEM_TEXTSTYLE_SHADOWED, &cgs.media.limboFont1 );
...
// -----------------------------------------------------------------------------
	...
	if(cg.hud.xp[0] >= 0) {
		...
		//maxw = CG_Text_Width_Ext( "32768", scaleVal, 0, &cgs.media.limboFont1  );
		maxw = CG_Text_Width_Ext( "32768", scaleVal, 0, &cgs.media.limboFont1 );
		//hudeditw = maxw + CG_Text_Width_Ext( "XP", scaleVal * 0.8f, 0, &cgs.media.limboFont1 ) + 2;
		ax = CG_AlignedX( cg.hud.xp[0], w, maxw, 0, qfalse ); // no hudeditw cos when theres no center it doesn't matter

		CG_Text_Paint_Ext( ax, cg.hud.xp[1], scaleVal, scaleVal, clr, str, 0, 0, ITEM_TEXTSTYLE_SHADOWED, &cgs.media.limboFont1 );
		CG_Text_Paint_Ext( ax + w + 2, cg.hud.xp[1], scaleVal * 0.8f, scaleVal * 0.8f, clr, "XP", 0, 0, ITEM_TEXTSTYLE_SHADOWED, &cgs.media.limboFont1 );
	}

	if(cg.hud.xpadd[0] >= 0) {
#define FWAITTIME 1000
#define FFADETIME 1000
		float width2;
		int maxw2;
		int ax2;
		char *addxpmsg;
		vec4_t colorfade = { 1.f, 1.f, 1.f, 1.f };
		static int lastxp = 0;
		int hudeditw;
		int xpmod;

		scaleVal = cg.hud.xpadd[2]/100.f;
		maxw2 = CG_Text_Width_Ext( "32768", scaleVal, 0, &cgs.media.limboFont1 );
		hudeditw = maxw2 + CG_Text_Width_Ext( "XP", scaleVal * 0.8f, 0, &cgs.media.limboFont1 ) + 2;
		xpmod = -(lastxp - ((32768 * cg.snap->ps.stats[STAT_XP_OVERFLOW])+cg.snap->ps.stats[STAT_XP]));
		addxpmsg = va( "%s%i", (xpmod > 0) ? "+" : "^1", xpmod );
		width2 = CG_Text_Width_Ext( addxpmsg, scaleVal, 0, &cgs.media.limboFont1 );
		ax2 = CG_AlignedX( cg.hud.xpadd[0], width2, maxw2, hudeditw, qtrue );

		if( cg.time - cg.xpChangeTime < (FWAITTIME + FFADETIME) ) {


			if( cg.time - cg.xpChangeTime > FWAITTIME ) {
				//(cg.time - cg.xpChangeTime - FWAITTIME) // 0 to FFADETIME
				colorfade[3] = (1.f - ((float)(cg.time - cg.xpChangeTime - FWAITTIME) / FFADETIME));
				if( colorfade[3] < 0.f )
					colorfade[3] = 0.f;
			}
		
			CG_Text_Paint_Ext( ax2, cg.hud.xpadd[1], scaleVal, scaleVal, colorfade, addxpmsg, 0, 0, ITEM_TEXTSTYLE_SHADOWED, &cgs.media.limboFont1 );
			CG_Text_Paint_Ext( ax2 + width2 + 2, cg.hud.xpadd[1], scaleVal * 0.8f, scaleVal * 0.8f, colorfade, "^3XP", 0, 0, ITEM_TEXTSTYLE_SHADOWED, &cgs.media.limboFont1 );
		} else {
			lastxp = ((32768 * cg.snap->ps.stats[STAT_XP_OVERFLOW])+cg.snap->ps.stats[STAT_XP]);
		}
	}
...
// codePRK HUDBackgrounds
static void CG_DrawHudBackgrounds( void ) {
	int i;
	int x,y,w,h;
	...
	for( i=0; i<MAX_HUD_BGS; i++ ) {
		...
		if(cg.hud.backgrounds[i][0] >= 0) {
			...
			x = cg.hud.backgrounds[i][0];
			y = cg.hud.backgrounds[i][1];
			w = cg.hud.backgrounds[i][2];
			h = cg.hud.backgrounds[i][3];

			//CG_FillRect( 4, height - 53, 140, 50, HUD_Background );
			//CG_DrawRect_FixedBorder( 4, height - 53, 140, 50, 1, HUD_Border );
			CG_FillRect( x, y, w, h, HUD_Color );
			CG_DrawRect_FixedBorder( x, y, w, h, 1, HUD_Border );
		}
	}
}


// codePRK visual weapon selection (thx chaos)
void CG_DrawCarryWeapons() {
	int time = 500;
	float color;
	vec4_t	colorON =  {0.8, 0.8, 0.8, 0.8};
	vec4_t	colorOFF = {0.5, 0.5, 0.5, 0.5};
	int i, w;
	int mainscale = 25;
	int size, scale;
	int tempx, tempy;
	int x; //11
	int x2 = SCREEN_WIDTH - 24;
	int y = SCREEN_HEIGHT - 140;
//	int y2 = 212;
	qboolean weaponbool = qfalse;
	qboolean weapcheck = qfalse;

	if ( cg_visWeapSelectTime.integer <= 0 )
		return;

	if (!cg.carryWeaponTime)
		return;


// --------------------------------------------------------
	//color = CG_FadeColor( cg.carryWeaponTime, 500 );
	if ( (cg.time - cg.carryWeaponTime) >= time ) {
		cg.carryWeaponTime = 0;
		return;
	}
	// fade out
	if ( time - (cg.time - cg.carryWeaponTime) < FADE_TIME ) {
		color = ( 500 - (cg.time - cg.carryWeaponTime) ) * 1.0/FADE_TIME;
	} else {
		color = 1.0;
	}
	if (!color) {
...
// --------------------------------------------------------
	...
	for ( i=1; i<9; i++ ) {
		...
		for ( w=0; w<MAX_WEAPS_IN_BANK_MP; w++ ) {
			int weapon = weapBanksMultiPlayer[i][w];
			int icon;
			...
			if (COM_BitCheck(cg.snap->ps.weapons, weapon)) {
				...
				size = weaponIconScale(weapon);
				//if( scale < mainscale * size )
				scale = mainscale * size;
				...
				if ( cg.predictedPlayerState.weapon == weapon ) {
					trap_R_SetColor(colorON);
					weaponbool = qtrue;
					weapcheck = qtrue;
				} else {
					trap_R_SetColor(colorOFF);
				}

				CG_DrawPic( tempx, tempy, scale, 20, icon );
				//CG_DrawPic( tempx+scale-5, y-5, 6, 6, cgs.media.numberShaders[i] );
			}
		}
		//if( y != y2 ) {
		if( x != x2 ) {
			...
			CG_DrawPic( x2 + 6, y+6, 9, 9, cgs.media.numberShaders[i] );

			//x += scale + 5; // next column
			y -= 24; // next row
		}
	}
	if( !weapcheck ) {
...
// ---------------------------------------------
	...
	//CG_Text_Paint_Ext( 16, 16, 0.25f, 0.25f, colorWhite, 
	//	va("%f %f %f", cg.snap->ps.viewangles[0], cg.snap->ps.viewangles[1], cg.snap->ps.viewangles[2]), 
...
void CG_drawCam( float x, float y, float width, float height, vec3_t vieworg, vec3_t vieang, float fov_x, float fov_y )
{
	refdef_t refdef;
	float x2, y2, w2, h2;
...
// -----------------------------------
	...
	// origin
	VectorCopy( vieworg, refdef.vieworg );
	// angle

	AnglesToAxis( vieang, refdef.viewaxis );
	// fov
	refdef.fov_x = fov_x;
	refdef.fov_y = fov_y;
...
// ======================================================
	...
	cg.refdef_current = &refdef; // what does this do?
...
// ======================================================
	...
	// Draw border
	CG_DrawRect_FixedBorder( x2, y2, w2, h2, 1, colorMdGrey );
...
void CG_DrawCMapCamera( rectDef_t *rect ) 
{
	float			x, y, w, h;
	float			mapcenterx, mapcentery, mapw, maph;
	float			mapz;
	vec3_t			origin;
	vec3_t			angles;
	...
	//CG_Text_Paint_Ext(x + 6, y + h - 10 , 0.24f, 0.24f, colorWhite, 
	//	va("%f %f %f %f", cg.mapcoordsMaxs[0], cg.mapcoordsMaxs[1], cg.mapcoordsMins[0], cg.mapcoordsMins[1]), 
	//	0, 0, 0, &cgs.media.font3 );

	if( !cg.mapcoordsMaxs[0] && !cg.mapcoordsMaxs[1] && cg.mapcoordsMins[0] == -128.f && cg.mapcoordsMins[1] == 128.f )
		return;

	// map coords
	mapw = ( cg.mapcoordsMaxs[0] - cg.mapcoordsMins[0] );
	maph = ( cg.mapcoordsMaxs[1] - cg.mapcoordsMins[1] );

	if (mapw > maph) {	// more width
		mapz = mapw;
	} else {			// more height
		mapz = maph;
	}

	mapcenterx = ( cg.mapcoordsMins[0] + ( mapw/2 ) );
	mapcentery = ( cg.mapcoordsMins[1] + ( maph/2 ) );

	VectorSet( origin, mapcenterx, mapcentery, mapz );

	angles[0] = 90; // pitch
	angles[1] = 0;  // yaw
	angles[2] = -90; // roll

	CG_drawCam( x, y, w, h, origin, angles, 54.f, 54.f );


	CG_DrawMapOverlaySimple( rect->x, rect->y, rect->w, rect->h ); // not the adjusted coordinates
}


#ifdef GS_RIFLEHACK // rifle

void CG_rifleCam( void )
{
	vec3_t camAngle;
	vec3_t camOrg;
	int x, y, w, h;
	...
	// origin
	VectorCopy( cg.lastImpact, camOrg );
	camOrg[ROLL] += distance;
	// angle
	VectorCopy( cg.refdefViewAngles, camAngle );
	camAngle[PITCH] = 90; // down	//(cg.snap->ps.viewangles[PITCH] / 9 ) + 90;

	// Draw the riflecam
	CG_drawCam( x, y, w, h, camOrg, camAngle, 30, 30 );

	// Draw impact time	
	CG_Text_Paint_Ext(x + 8, y + h - 8 , 0.24f, 0.24f, colorWhite, va("^d%.1f ^7s", (float)cg.flyTime / 1000.0f ), 0, 0, 0, &cgs.media.font3 );
}
...
void CG_DrawGrenadelauncherTrace( void )
{
	#define TIME_STEPP 50
	#define TIME_OFFSET 12 // correction
	vec3_t forward, right, up;
	vec3_t muzzlePoint, start;
	vec3_t lastPos;
	vec3_t rifleImpact;
	trajectory_t rifleTrajectory;
	trace_t		tr;
	int timeOffset = 0;
	int maxTime = 3250 + 850; //3250;
	int totalFly = 0;
	qboolean once = qfalse;
	qboolean second = qfalse;
	int insky = 0;
	vec3_t color;
	int trailtimemult = 8;
	...
	if (cg.refdef.viewaxis[0] == NULL)
		return;

	//if (cg.snap->ps.weaponTime != 0 )
	//	return;

	if( !( cg.snap->ps.weapon == WP_GPG40 || cg.snap->ps.weapon == WP_M7 ) ) {
		return;
	}
...
// ##############################################################################################
	...
	//VectorCopy( cg.refdef.vieworg, muzzlepos );
	//if(cg.snap->ps.viewangles[PITCH] > 30.0 && (cg.snap->ps.eFlags & EF_CROUCHING))
	//	muzzlepos[2] += 30.0 - cg.snap->ps.viewheight;
	VectorCopy( cg.snap->ps.origin, muzzlePoint );
	if(cg.snap->ps.viewangles[PITCH] > 30 && 
		(cg.snap->ps.eFlags & EF_CROUCHING)) {
		muzzlePoint[2] += 30;
	}
	...
	VectorScale(forward, 2000, forward);
		...
		VectorCopy(start, start_back);
		VectorCopy(forward, forward_back);


	//	VectorCopy(start_back, start);
	//	VectorCopy(forward_back, forward);
// ##############################################################################################

	rifleTrajectory.trType = TR_GRAVITY;
	rifleTrajectory.trTime = cg.time + TIME_OFFSET;
	VectorCopy( start, rifleTrajectory.trBase );
...
// ##############################################################################################
	...
	while (timeOffset < maxTime) {
		...
		BG_EvaluateTrajectory(&rifleTrajectory, cg.time + timeOffset, nextPos, qfalse, 0);
		CG_Trace(&tr, rifleImpact, 0, 0, nextPos, cg.snap->ps.clientNum, MASK_MISSILESHOT ); // MASK_MISSILESHOT
...
// --------------------------------------------------------
		//surfcheck = (tr.surfaceFlags & SURF_NODRAW || tr.surfaceFlags & SURF_NOIMPACT || tr.surfaceFlags & SURF_SKY);

		if( insky && (tr.fraction != 1) ) {
			trace_t		tr2;
			vec3_t nextPoso, nextPosn;
// --

			// trace back for sky
			VectorCopy(nextPos, nextPoso);
			VectorSubtract( rifleImpact, nextPos, nextPosn );
			VectorAdd( nextPos, nextPosn, nextPosn );
			// nextPosn[2] = 0;
			//VectorScale( nextPosn, 20.f, nextPosn );
			CG_Trace(&tr2, nextPoso, 0, 0, nextPosn, cg.snap->ps.clientNum, MASK_MISSILESHOT );
			//CG_RailTrail_Ext( nextPoso, nextPosn, qfalse, colorBlue, cg.frametime*4 );

			if( tr2.surfaceFlags & SURF_SKY ) {
				insky = 0;
				tr.fraction = 1; // not yet
			}
// --
		}
...
// --------------------------------------------------------
		...
		if( ( tr.fraction != 1 )
			...
			//VectorCopy(tr.endpos, nextPos);
			VectorCopy(tr.endpos, lastPos);

			// When a nade flies for over 750ms and hits somnexing, it'll explode
			if (totalFly > 750) {
				// create a green end, so we know it hits the floor
				CG_RailTrail_Ext( rifleImpact, lastPos, qfalse, colorGreen, cg.frametime*(trailtimemult*2) );
				break;
			}

			CG_RailTrail_Ext( rifleImpact, lastPos, qfalse, color, cg.frametime*trailtimemult );

			// calculate reflect angle (forward axis)
			hitTime = cg.time + totalFly - TIME_STEPP + TIME_STEPP * tr.fraction;
			BG_EvaluateTrajectoryDelta(&rifleTrajectory, hitTime, rifleTrajectory.trDelta, qfalse, 0);
			dot = DotProduct(rifleTrajectory.trDelta, tr.plane.normal);
			VectorMA(rifleTrajectory.trDelta, -2*dot, tr.plane.normal, rifleTrajectory.trDelta);
			...
			if( second ) {
				VectorScale(rifleTrajectory.trDelta, 1.70f, rifleTrajectory.trDelta);
			}
			second = qtrue;
			...
			// new base origins 
			VectorCopy(tr.endpos, rifleTrajectory.trBase);
			VectorAdd( rifleTrajectory.trBase, tr.plane.normal, rifleTrajectory.trBase );
...
// --------------------------------------------------------------------------
			if( 1 ) {
				...
				VectorCopy( lastPos, tstart );
				VectorCopy( nextPos, tend );

				//CG_RailTrail( NULL, tstart, tend, 0 );
				//CG_RailTrail_Ext( tstart, tend, qfalse, colorYellow, cg.frametime*3  );

				CG_Trace(&t, cg.refdef.vieworg, NULL, NULL, tend, 0, MASK_SHOT);

				if(insky)
					VectorCopy( colorBlue, color );
				else if(t.fraction == 1.f)
					VectorCopy( colorYellow, color );
				else
					VectorCopy( colorRed, color );

       			CG_RailTrail_Ext( tstart, tend, qfalse, color, cg.frametime*trailtimemult );
			}
// --------------------------------------------------------------------------
	...
	VectorCopy( lastPos, cg.lastImpact );
	cg.flyTime = totalFly;
...
void CG_DrawMissileCamera(rectDef_t *rect ) 
{
	...
	// origin
	VectorCopy( cent->lerpOrigin, vieworg );
	vieworg[2] += 8; // up
	AngleVectors( cg.snap->ps.viewangles, forward, NULL, NULL );
	VectorMA( vieworg, -16, forward, vieworg ); // back

	// angles
	VectorCopy( cg.snap->ps.viewangles, viewang );
	// viewangle[2]	= 0;

	CG_drawCam( x, y, w, h, vieworg, viewang, cg.refdef_current->fov_x, cg.refdef_current->fov_y );
