...
vmCvar_t cg_drawObjectiveFP;
vmCvar_t cg_drawFireteamIcons;
vmCvar_t cg_drawPickupMsg;
vmCvar_t cg_popupDelay;
vmCvar_t cg_shadowedPopups;
vmCvar_t cg_numPopups;
vmCvar_t cg_hudColor;
vmCvar_t cg_hudAlpha;
vmCvar_t cg_drawHardcore;


vmCvar_t cg_visWeapSelectTime;
...
// codePRK adminpanel
vmCvar_t	ui_adminLevel;
vmCvar_t	ui_adminFlags;
...
// codePRK CVARs client
	...
	{ &cg_drawHardcore, "cg_drawHardcore", "1", CVAR_ARCHIVE },

	{ &cg_visWeapSelectTime, "cg_visWeapSelectTime", "2000", CVAR_ARCHIVE },


	{ &cg_testVar, "cg_testVar", "0", CVAR_ARCHIVE },


	{ &cg_x, "cg_x", "0", CVAR_ARCHIVE },


	{ &cg_hitSounds, "cg_hitSounds", "1", CVAR_ARCHIVE }, // | CVAR_USERINFO

	// codePRK adminpanel
	{ &ui_adminLevel, "ui_adminLevel", "0", CVAR_TEMP | CVAR_ROM },
	{ &ui_adminFlags, "ui_adminFlags", "", CVAR_TEMP | CVAR_ROM },
...
vec4_t	HUD_Color	= { 0.16f,  0.2f,   0.17f, 0.8f };
...
void jP_SetHUDColors(void)
{
	...
	char	*nextVal;
	char	color[MAX_CVAR_VALUE_STRING];

	Q_strncpyz( color, cg_hudColor.string, sizeof(cg_hudColor.string) );

	if( !Q_stricmp( color, "default") || !Q_stricmp( color, "def") ) {

		trap_Cvar_Set( "cg_hudColor", ".16 .2 .17");

		return;
	} else if (!Q_stricmp( color, "red")) {
		trap_Cvar_Set( "cg_hudColor", ".4 .0 .0");
		return;
	} else if (!Q_stricmp( color, "green")) {
		trap_Cvar_Set( "cg_hudColor", ".0 .4 .0");
		return;
	} else if (!Q_stricmp( color, "blue")) {
		trap_Cvar_Set( "cg_hudColor", ".0 .0 .4");
		return;
	} else if (!Q_stricmp( color, "black")) {
		trap_Cvar_Set( "cg_hudColor", ".0 .0 .0");
		return;
	} else if (!Q_stricmp( color, "white")) {
		trap_Cvar_Set( "cg_hudColor", "1.0 1.0 1.0");
		return;
	} else if (!Q_stricmp( color, "dkgrey") || !Q_stricmp( color, "darkgrey") ||
		       !Q_stricmp( color, "dkgray") || !Q_stricmp( color, "darkgray")) {
		trap_Cvar_Set( "cg_hudColor", ".25 .25 .25");
		return;
	} else if (!Q_stricmp( color, "grey")   || !Q_stricmp( color, "gray")) {
		trap_Cvar_Set( "cg_hudColor", ".5 .5 .5");
		return;
	}

	nextVal = strtok(color, " ");

	for (count = 0; count < 3 && nextVal; count++) 
	{
		newColor[count] = atof(nextVal);
		nextVal		   = strtok(NULL, " ,");
	}

	HUD_Alpha = cg_hudAlpha.value;

	Vector4Set(HUD_Border, 0.5, 0.5, 0.5, (HUD_Alpha/1.6f)); //0.5f); // HUD_Alpha
	Vector4Set(HUD_Color, newColor[0],   newColor[1],   newColor[2], HUD_Alpha); //0.8f); // HUD_Alpha	   
}

void CG_shadowCheck( void ) {
	if( cg_shadows.value > 1 ) {
		trap_Cvar_Set("cg_shadows", "1" );
	} else if ( cg_shadows.value < 0 ) {
	...
	for ( i = 0, cv = cvarTable ; i < cvarTableSize ; i++, cv++ ) {
		if(cv->vmCvar) {
			...
			if(cv->modificationCount != cv->vmCvar->modificationCount) {
				   ...
				   cv->vmCvar == &cg_autoactivate || cv->vmCvar == &cg_predictItems)
				{
					...
					switch( cg_hitSounds.integer ) {
						case 1:
							smode =  "on";				break;
						case 2:
							smode =  "headshots and team"; 	break;
						case 3:
							smode =  "^5enemies only";	break;
						case 4:
							smode =  "^5headshots only";	break;
						case 5:
							smode =  "team only";		break;
						default:
							smode =  "^1off";				break;
					}

					CG_Printf("^3hitSounds:^2 %s\n", smode );
				}
				...
				// codePRK cvar change
				else if(cv->vmCvar == &cg_hudColor) {
						jP_SetHUDColors();
				}
				else if ( cv->vmCvar == &cg_shadows	) {
					CG_shadowCheck();
				}
...
qboolean CG_BackupProfile(void)
{
	...
	Com_sprintf(bakPath, sizeof(bakPath),
		"profiles/%s/%s.backup", profile, CONFIG_NAME);
...
*/
	...
	cgs.media.headShot	= trap_S_RegisterSound( "sound/etfrag/hithead.wav",	qfalse );
	cgs.media.bodyShot	= trap_S_RegisterSound( "sound/etfrag/hit.wav",		qfalse );
	cgs.media.teamShot	= trap_S_RegisterSound( "sound/etfrag/hitteam.wav",	qfalse );
...
" );
	...
	trap_R_LoadDynamicShader( "compassColorTestORANGE",
"compassColorTestORANGE\n\
{\n\
	{\n\
		map $whiteimage\n\
...
" );
	trap_R_LoadDynamicShader( "compassColorTestBLUE",
"compassColorTestBLUE\n\
{\n\
	{\n\
		map $whiteimage\n\
		blendFunc GL_ONE_MINUS_DST_COLOR GL_ZERO\n\
...
" );
	cgs.media.compassColorTest_oran = trap_R_RegisterShaderNoMip( "compassColorTestORANGE" );
	cgs.media.compassColorTest_blue = trap_R_RegisterShaderNoMip( "compassColorTestBLUE" );
...
//	cgs.media.commandCentreWoodShader		= trap_R_RegisterShaderNoMip( "ui/assets2/commandMap" );
	...
	cgs.media.SquareAutomapMaskShader = trap_R_RegisterShaderNoMip( "levelshots/automap_mask_square" );
	cgs.media.compass3 =				trap_R_RegisterShaderNoMip( "gfx/2d/compass3.tga" );
	...
	cgs.media.rframe = trap_R_RegisterShaderNoMip( "gfx/limbo/rframe" );
...
//	cgs.media.hflakWeaponSnd =		trap_S_RegisterSound( "sound/vehicles/misc/20mm_fire.wav", qfalse );
	...
	trap_R_RegisterFont( "courbd", 30, &cgs.media.font3 );
	trap_R_RegisterFont( "courbd", 30, &cgs.media.font4 );
	...
	cgs.media.transWhite =					trap_R_RegisterShader( "textures/sfx/transWhite" );
	cgs.media.transRed =					trap_R_RegisterShader( "textures/sfx/transRed" );
	cgs.media.transGreen =					trap_R_RegisterShader( "textures/sfx/transGreen" );
	cgs.media.transBlue =					trap_R_RegisterShader( "textures/sfx/transBlue" );

	cgs.media.gsfloortest =					trap_R_RegisterShaderNoMip( "textures/egypt_floor_sd/marblefloor_c04" );
	cgs.media.gsskytest =					trap_R_RegisterShaderNoMip( "etfrag/limbosky" );
