...
// codePRK adminsystem popups

//#define ADMINSYS_WINDOW_X		16
#define ADMINSYS_WINDOW_X		(320-(510/2)) // centered
#define ADMINSYS_WINDOW_Y		16
#define ADMINSYS_WINDOW_WIDTH	510
#define ADMINSYS_WINDOW_HEIGHT	348



#define NUM_PM_STACK_ITEMS	32
typedef struct as_pmStackItem_s as_pmListItem_t;
struct as_pmStackItem_s {
	qboolean				inuse;
	int						time;
	char					message[128];
	as_pmListItem_t*			next;
};
as_pmListItem_t		as_cg_pmStack[NUM_PM_STACK_ITEMS];
as_pmListItem_t*	as_cg_pmOldList;
as_pmListItem_t*	as_cg_pmWaitingList;

#define WAITTIME 6000
#define FADETIME 2500

void UI_adminsys_UpdatePMLists( void ) {
	as_pmListItem_t* listItem;
	as_pmListItem_t* lastItem;

	if ((listItem = as_cg_pmWaitingList)) {
		int t = listItem->time;
		if( DC->realTime > t ) {
			if( listItem->next ) {
				// there's another item waiting to come on, so move to old list
				as_cg_pmWaitingList = listItem->next;
				as_cg_pmWaitingList->time = DC->realTime; // set time we popped up at

				//CG_AddToListFront( &as_cg_pmOldList, listItem );
				listItem->next = as_cg_pmOldList;
				as_cg_pmOldList = listItem;
			} else {
				if( DC->realTime > t + WAITTIME + FADETIME ) {
					// we're gone completely
					as_cg_pmWaitingList = NULL;
	...
	listItem = as_cg_pmOldList;
	lastItem = NULL;
	while( listItem ) {
		int t = (listItem->time + WAITTIME + FADETIME);
		if( DC->realTime > t ) {
...
as_pmListItem_t* UI_FindFreePMItem( void ) {
	as_pmListItem_t* listItem;
	as_pmListItem_t* lastItem;

	int i = 0;
	for( ; i < NUM_PM_STACK_ITEMS; i++ ) {
		if( !as_cg_pmStack[i].inuse ) {
			return &as_cg_pmStack[i];
		}
	}

	// no totally free items, so just grab the last item in the oldlist
	if ((lastItem = listItem = as_cg_pmOldList)) {
		while( listItem->next ) {
			lastItem = listItem;
			listItem = listItem->next;			
		}

		if( lastItem == as_cg_pmOldList ) {
			as_cg_pmOldList = NULL;
		} else {
			lastItem->next = NULL;
		}
...
void UI_DrawPMItem( as_pmListItem_t* listItem, int x, int y ) {
	...
	t = listItem->time + WAITTIME;
	if( DC->realTime > t ) {
		fadeing = ((DC->realTime - t) / (float)FADETIME);
		if( fadeing < 1 ) {
			colourText[3] = 1 - fadeing;
		} else {
			colourText[3] = 0.f;
		}
	} else {
	...
	DC->drawText( x, y, scaleVal, colourText, listItem->message, 0, 0, 0 );
}

void UI_adminsys_DrawPopups( void ) {
	//vec4_t color = { 0.f, 0.f, 0.f, 0.8f };
	int i;
	as_pmListItem_t* listItem = as_cg_pmOldList;
	int y;
	int x;
	int size;
	int numpopups;

	if( !as_cg_pmWaitingList ) {
		return;
	}


	size = 12; // dis between msgs
	numpopups = 4; // +1 is result
	y = (480 - 16 - 10);
	x = ADMINSYS_WINDOW_X + 10;

	UI_DrawPMItem( as_cg_pmWaitingList, x, y );

	for( i = 0; i < numpopups && listItem; i++, listItem = listItem->next ) {
		y -= size + 2;

		UI_DrawPMItem( listItem, x, y );
	}
}


void UI_adminsys_DrawPinfo( void )
{
	float x = ADMINSYS_WINDOW_X + ((120*2) + (6*3));
	float y = ADMINSYS_WINDOW_Y + 76;
	int index = uiInfo.playerIndex;
	char name[6+13]; // less cos small box
	char name2[13];
	char slot[6+3];
	char team[6+12];
	char misc1[8];
	...
	Q_strncpyz( name2, uiInfo.playerNames[index], sizeof(name2) );
	if( strlen(name2) >= 12 ) {
		int strl = strlen(name2);
		name2[strl-1] = name2[strl-2] = name2[strl-3] = '.';
	}
	Com_sprintf( name, sizeof(name), "name: %s", name2 );
	Com_sprintf( slot, sizeof(slot), "slot: %d", uiInfo.playerNumbers[index] );
	Com_sprintf( team, sizeof(team), "team: %s", uiInfo.playerTeamNumbers[index] == TEAM_AXIS ? "^1Axis" : 
						(uiInfo.playerTeamNumbers[index] == TEAM_ALLIES ? "^4Allies" : "^2Spectator") );

	
	Com_sprintf( misc1, sizeof(misc1), "%s", uiInfo.playerMuted[index] ? "^1Muted" : "" );
	Com_sprintf( misc2, sizeof(misc2), "%s", uiInfo.playerRefereeStatus[index] == RL_RCON ? "^1RCON" : 
						(uiInfo.playerRefereeStatus[index] == RL_REFEREE ? "^3REFEREE" : "") );

	x += 6;

	y += 14;
	DC->drawText( x, y, 0.2f, colorWhite, name, 0, 0, 0 );

	y += 14;
	DC->drawText( x, y, 0.2f, colorWhite, slot, 0, 0, 0 );

	y += 14;
	DC->drawText( x, y, 0.2f, colorWhite, team, 0, 0, 0 );

	y += 14;
	DC->drawText( x, y, 0.2f, colorWhite, misc1, 0, 0, 0 );

	if( misc1[0] ) {
		y += 14;
	}
	DC->drawText( x, y, 0.2f, colorWhite, misc2, 0, 0, 0 );
}
...
void Window_Paint(Window *w, float fadeAmount, float fadeClamp, float fadeCycle) {
  ...
  else if (w->style == 1337) {
	  // two gradient bars each side

	  rectDef_t r = fillRect;
	  r.w /= 4;
	  GradientBar_Paint(&r, w->backColor);
	  r.x += (r.w * 3);
	  r.w *= -1;
	  GradientBar_Paint(&r, w->backColor);
  }
	...
	if( Int_Parse(args, &useprofile) ) {
		...
		if( useprofile && cl_profileStr[0] ) {
			...
			Com_sprintf(bakPath, sizeof(bakPath),
				"profiles/%s/%s.backup", cl_profileStr, CONFIG_NAME);
...
// --------------------------------------------------------------------
		// codePRK adminpanel flags
		if (item->enableFlag) {
			char cvarVal[1024];
			char enableFlag;
			DC->getCVarString(item->cvarTest, cvarVal, sizeof(cvarVal));
...
// codePRK adminpanel
//char ui_adminsysPlayerName[MAX_NAME_LENGTH];
//int ui_adminsysLevel;
static char *message_hack;
...
// ------------------------------------------------------------
	// codePRK adminpanel message_hack
	if( (item->window.flags & WINDOW_ADMINPANELHACK) ) {
		//char *pname;
		//int level;
		//char *msg;

		//pname = ui_adminsysPlayerName;
		//level = ui_adminsysLevel;

	//	msg = va( "^7Hello ^3%s^7, you are currently a level ^1%d^7 user.", pname, level );

		Q_strncpyz( text, message_hack, sizeof(text) );
...
// --------------------------------------
// codePRK adminpanel mute/unmute

	//if ( item->text == "mute" || item->text == "unmute" ) {
  if( (!Q_stricmp(item->text, "mute")) || (!Q_stricmp(item->text, "unmute")) ) {
		//if( uiInfo.playerMuted[uiInfo.playerCount] ) 
		if (uiInfo.playerIndex >= 0 && uiInfo.playerIndex < uiInfo.playerCount) {
			if( uiInfo.playerMuted[uiInfo.playerIndex] ) {
				if( !trx )
					trx = item->textRect.x + 3;
				item->text = "unmute";
				item->textRect.x = trx - 3;
			} else {
				if( !trx )
					trx = item->textRect.x;
				item->text = "mute";
...
void Menu_Paint(menuDef_t *menu, qboolean forcePaint) {
	...
	// codePRK adminpanel popups
	//if (menu->window.name == "ingame_admin") {
	if( !Q_stricmp(menu->window.name, "ingame_admin")  ) {
		UI_adminsys_DrawPopups();
		UI_adminsys_UpdatePMLists();
		UI_adminsys_DrawPinfo();
	}
	// xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
...
#define GROUP_NAME		"grpIngameAdmin"
/*
#define WINDOW_COLOR1	.16f
#define WINDOW_COLOR2	.2f
#define WINDOW_COLOR3	.17f
*/
#define WINDOW_COLOR1	.4f
#define WINDOW_COLOR2	0
#define WINDOW_COLOR3	0

#define BUTTONS_WIDTH	120
#define BUTTONS_HEIGHT	14

// ===================================================================================================
// tools

itemDef_t *adminpanel_addItem( menuDef_t *menu ) {
	...
	return menu->items[menu->itemCount];
}
void adminpanel_startItem( menuDef_t *menu, itemDef_t *item )
{
	//if (!Item_Parse(handle, menu->items[menu->itemCount])) {
	//	return qfalse;
	//}
	...
	if (menu->itemHotkeyMode && item->hotkey >= 0)
	{
		menu->onKey[item->hotkey] = String_Alloc(item->onKey);
	}
...
void ui_adminsys_addPopup( const char* message )
{
	as_pmListItem_t* listItem;
	...
	listItem = UI_FindFreePMItem();
	if( !listItem ) {
		return;
	}

	listItem->inuse = qtrue;
	Q_strncpyz( listItem->message, message, sizeof( as_cg_pmStack[0].message ) );
	...
	if( !as_cg_pmWaitingList ) {
		as_cg_pmWaitingList = listItem;
		listItem->time = DC->realTime;
	} else {
		as_pmListItem_t* loop = as_cg_pmWaitingList;
		while( loop->next ) {
			loop = loop->next;
		}
	...
	//Com_Printf( "^6%s\n", message );
	//DC->drawText( 2, 2, 0.2f, colorWhite, "TESTFUNCTION", 1, 1, 1);
}
...
static const struct g_adminsys_cmd g_adminsys_cmds[] = {
	{"readconfig", 		'#',		"read all data saved in g_adminsysDir"			},
	{"writeconfig",		'$',		"write admins to g_adminsysDir"					},
	// ---
	{"test", 			'z',		"testing cmd"									},
	{"pinfo", 			'y',		"display information about a player",			},
	// -------------------------------------------------------------------------------------------------------------------
	{"admintest", 		'a',		"display admin level"							},
	{"help", 			'b',		"show list of avaible cmds"						},

	{"setlevel", 		'c',		"sets the admin level of a player"				},
	{"levlist", 		'd',		"list of loaded admin levels"					},

	{"mute", 			'e',		"removes the chat capabilities of a player"		},
	{"unmute", 			'f',		"restores the chat capabilities of a player"	},
	{"kick", 			'g',		"kick player from the server"					},
	{"ban", 			'h',		"ban player from the server"					},

	//{"ma",			'~',		"write the adminchat",							},
	{"mc",				'+',		"the clanchat",									},
...
qboolean G_adminsys_permission(char flag)
{
	//int i;
	char *flags;
	...
	flags = "";
	DC->getCVarString( "ui_adminFlags", flags, sizeof(flags) );
	...
	return qfalse;
#else
	Com_Printf("DEBUG: ^7permission granted!");
	return qtrue;
#endif
}

qboolean G_adminsys_cmd_check(char *cmd)
{
	int i;
	...
	for (i=0; g_adminsys_cmds[i].keyword[0]; i++) {
		...
		if(G_adminsys_permission(g_adminsys_cmds[i].flag)) {
			return qtrue;
		}
		else {
			Com_Printf("%s: ^7permission denied",
...
char G_adminsys_cmd_flag(char *cmd)
{
	int i;
	...
	for (i=0; g_adminsys_cmds[i].keyword[0]; i++) {
		...
		return (g_adminsys_cmds[i].flag);
	}

	return 0;
}

int G_adminsys_cmd_id(char *cmd)
{
	int i;
...
void adminpanel_addWindow( menuDef_t *menu, float x, float y, float w, float h, char *WINDOW_TEXT, int GRADIENT_START_OFFSET )
{
	itemDef_t *item1, *item2, *item3, *item4;

	//adminpanel_setStr( "window", item1->window.name );
	item1 = adminpanel_addItem( menu );
		item1->window.name = "window";
		item1->window.group = GROUP_NAME;
		item1->window.rectClient.x = x;
		item1->window.rectClient.y = y;
		item1->window.rectClient.w = w;
		item1->window.rectClient.h = h;
		item1->window.style = WINDOW_STYLE_FILLED;
		item1->window.backColor[0] = item1->window.backColor[1] = item1->window.backColor[2] = .0f;
		item1->window.backColor[3] = .6f;
		item1->window.border = WINDOW_BORDER_FULL;
		item1->window.borderColor[0] = item1->window.borderColor[1] = item1->window.borderColor[2] = item1->window.borderColor[3] = 0.5f;
		item1->window.flags |= WINDOW_VISIBLE;
		item1->window.flags |= WINDOW_DECORATION;
	//Item_ValidateTypeData(item1);
	adminpanel_startItem( menu, item1 );

	item2 = adminpanel_addItem( menu );
		item2->window.name = "titlebar";
		item2->window.group = GROUP_NAME;
		item2->window.rectClient.x = x+2.f;
		item2->window.rectClient.y = y+2.f;
		item2->window.rectClient.w = (float)GRADIENT_START_OFFSET;
		item2->window.rectClient.h = 24.f;
		item2->window.style = WINDOW_STYLE_FILLED;
		item2->window.backColor[0] = WINDOW_COLOR1;
		item2->window.backColor[1] = WINDOW_COLOR2;
		item2->window.backColor[2] = WINDOW_COLOR3;
		item2->window.backColor[3] = .8f;
		item2->window.flags |= WINDOW_VISIBLE;
		item2->window.flags |= WINDOW_DECORATION;
	//Item_ValidateTypeData(item2);
	adminpanel_startItem( menu, item2 );

	item3 = adminpanel_addItem( menu );
		item3->window.name = "titlebargradient";
		item3->window.group = GROUP_NAME;
		// $evalint(GRADIENT_START_OFFSET+2) 2 $evalint(WINDOW_WIDTH-(GRADIENT_START_OFFSET+4)) 24
		item3->window.rectClient.x = x+(float)(GRADIENT_START_OFFSET+2);
		item3->window.rectClient.y = y+2.f;
		item3->window.rectClient.w = (float)(w-(GRADIENT_START_OFFSET+4));
		item3->window.rectClient.h = 24.f;
		item3->window.style = WINDOW_STYLE_GRADIENT;
		item3->window.backColor[0] = WINDOW_COLOR1;
		item3->window.backColor[1] = WINDOW_COLOR2;
		item3->window.backColor[2] = WINDOW_COLOR3;
		item3->window.backColor[3] = .8f;
		item3->window.flags |= WINDOW_VISIBLE;
		item3->window.flags |= WINDOW_DECORATION;
	//Item_ValidateTypeData(item3);
	adminpanel_startItem( menu, item3 );

	item4 = adminpanel_addItem( menu );
		item4->window.name = "windowtitle";
		item4->window.group = GROUP_NAME;
		// 2 2 $evalint(WINDOW_WIDTH-4) 24
		item4->window.rectClient.x = x+2.f;
		item4->window.rectClient.y = y+2.f;
		item4->window.rectClient.w = (float)(w-4);
		item4->window.rectClient.h = 24.f;
		item4->text = WINDOW_TEXT;
		item4->font = UI_FONT_ARIBLK_27;
		item4->textscale = .4f;
		item4->textalignx = 3.f;
		item4->textaligny = 20.f;
		item4->window.foreColor[0] = item4->window.foreColor[1] = item4->window.foreColor[2] = .6f;
		item4->window.foreColor[3] = 1.f;
		item4->window.flags |= WINDOW_FORECOLORSET;
		item4->window.border = WINDOW_BORDER_FULL;
		item4->window.borderColor[0] = item4->window.borderColor[1] = item4->window.borderColor[2] = .1f;
		item4->window.borderColor[3] = .2f;
		item4->window.flags |= WINDOW_VISIBLE;
		item4->window.flags |= WINDOW_DECORATION;

	//Item_ValidateTypeData(item4);
	adminpanel_startItem( menu, item4 );
}

void adminpanel_addBackground( menuDef_t *menu, float GRADIENT_OFFSET )
{
	itemDef_t *background;
	itemDef_t *gradient;

	background = adminpanel_addItem( menu );
		background->window.name = "adminsys_background";
		background->window.group = GROUP_NAME;
		background->window.rectClient.x = -menu->window.rect.x;
		background->window.rectClient.y = -menu->window.rect.y;
		background->window.rectClient.w = 640;
		background->window.rectClient.h = 480;
		background->window.style = WINDOW_STYLE_FILLED;
		background->window.backColor[0] = background->window.backColor[1] = background->window.backColor[2] = .0f;
		background->window.backColor[3] = .5f;
		background->window.flags |= WINDOW_VISIBLE;
		background->window.flags |= WINDOW_DECORATION;
	adminpanel_startItem( menu, background );

	gradient = adminpanel_addItem( menu );
		gradient->window.name = "adminsys_backgroundgradient";
		gradient->window.group = GROUP_NAME;
		gradient->window.rectClient.x = -menu->window.rect.x;
		gradient->window.rectClient.y = -menu->window.rect.y;
		//gradient->window.rectClient.w = GRADIENT_OFFSET;
		gradient->window.rectClient.w = 640;
		gradient->window.rectClient.h = 480;
		gradient->window.style = 1337; //WINDOW_STYLE_GRADIENT;
		gradient->window.backColor[0] = 0.0f;
		gradient->window.backColor[1] = 0.0f;
		gradient->window.backColor[2] = 0.1f;
		gradient->window.backColor[3] = .5f;
		gradient->window.flags |= WINDOW_VISIBLE;
		gradient->window.flags |= WINDOW_DECORATION;
	adminpanel_startItem( menu, gradient );
}


void adminpanel_addWindow_blank( menuDef_t *menu, float x, float y, float w, float h )
{
	itemDef_t *item;

	//adminpanel_setStr( "window", item1->window.name );
	item = adminpanel_addItem( menu );
		item->window.name = "window_blank";
		item->window.group = GROUP_NAME;
		item->window.rectClient.x = x;
		item->window.rectClient.y = y;
		item->window.rectClient.w = w;
		item->window.rectClient.h = h;
		item->window.style = WINDOW_STYLE_FILLED;
		item->window.backColor[0] = item->window.backColor[1] = item->window.backColor[2] = .0f;
		item->window.backColor[3] = .6f;
		item->window.border = WINDOW_BORDER_FULL;
		item->window.borderColor[0] = item->window.borderColor[1] = item->window.borderColor[2] = item->window.borderColor[3] = 0.5f;
		item->window.flags |= WINDOW_VISIBLE;
		item->window.flags |= WINDOW_DECORATION;
	//Item_ValidateTypeData(item1);
	adminpanel_startItem( menu, item );
}

void adminpanel_addText_small( menuDef_t *menu, char *text, float x, float y )
{
	itemDef_t *item;

	item = adminpanel_addItem( menu );
		item->window.name = "aptext";
		item->window.group = GROUP_NAME;
		// 2 2 $evalint(WINDOW_WIDTH-4) 24
		item->window.rectClient.x = x;
		item->window.rectClient.y = y;
		//item->window.rectClient.w = (7.f * strlen(text));
		//item->window.rectClient.h = 12.f;
		item->window.rectClient.w = 0.f;
		item->window.rectClient.h = 0.f;
		item->type = ITEM_TYPE_TEXT;
	Item_ValidateTypeData(item);
		item->text = text;
		item->font = UI_FONT_COURBD_30;
		item->textscale = .2f;
		item->textalignx = 3.f;
		item->textaligny = 9.f;
		item->window.foreColor[0] = item->window.foreColor[1] = item->window.foreColor[2] = .6f;
		item->window.foreColor[3] = 0.9f;
		item->window.flags |= WINDOW_FORECOLORSET;
		//item->window.border = WINDOW_BORDER_FULL;
		//item->window.borderColor[0] = item->window.borderColor[1] = item->window.borderColor[2] = .8f;
		//item->window.borderColor[3] = .4f;
		item->window.flags |= WINDOW_VISIBLE;
		item->window.flags |= WINDOW_DECORATION;
...
void adminpanel_addButton( menuDef_t *menu, float x, float y, float w, float h, 
						  char *text, float textscale, int aligny, char *action, char *tooltip )
{
	itemDef_t *button;
	char *wname = va("bttn%s", text);

	button = adminpanel_addItem( menu );

	button->window.name = wname;
	button->window.group = GROUP_NAME;
	button->window.rectClient.x = (float)x;
	button->window.rectClient.y = (float)y;
	button->window.rectClient.w = (float)w;
	button->window.rectClient.h = (float)h;
	button->type = ITEM_TYPE_BUTTON;
	Item_ValidateTypeData(button);
	button->text = text;
	button->font = UI_FONT_COURBD_30;
	button->textscale = textscale;
	button->textalignment = ITEM_ALIGN_CENTER;
	button->textalignx = (float)0.5 * w;
	button->textaligny = (float)aligny;
	button->window.style = WINDOW_STYLE_FILLED;
	button->window.backColor[0] = button->window.backColor[1] = button->window.backColor[2] = .3f;
	button->window.backColor[3] = .4f;
	button->window.foreColor[0] = button->window.foreColor[1] = button->window.foreColor[2] = .6f;
	button->window.foreColor[3] = 1.f;
	button->window.flags |= WINDOW_FORECOLORSET;
	button->window.border = WINDOW_BORDER_FULL;
	button->window.borderColor[0] = button->window.borderColor[1] = button->window.borderColor[2] = .1f;
	button->window.borderColor[3] = .5f;
	button->window.flags |= WINDOW_VISIBLE;

	button->mouseEnter = va("\
\"setitemcolor\" \"%s\" \"forecolor\" .9 .9 .9 1 ; \
\"setitemcolor\" \"%s\" \"backcolor\" .5 .5 .5 .4\
", wname, wname);

	button->mouseExit = va("\
\"setitemcolor\" \"%s\" \"forecolor\" .6 .6 .6 1 ; \
\"setitemcolor\" \"%s\" \"backcolor\" .3 .3 .3 .4\
", wname, wname);
...
", wname, wname, action );
	...
	if( Item_ValidateTooltipData(button) ) {
		button->toolTipData->text = tooltip;
	}

	adminpanel_startItem( menu, button );
}


void adminpanel_addButton_adminsys( menuDef_t *menu, float x, float y, float w, float h, 
								char *text, float textscale, int aligny, char *action, qboolean uiScript )
{
	itemDef_t *button;
	char *wname = va("bttn%s", text);
	int cmdid;

	cmdid = G_adminsys_cmd_id(action);
	if( cmdid < 0 )
		return;
...
\"play\" \"sound/menu/select.wav\" ; \
%s \"!%s\"\
", wname, wname, uiScript ? "uiScript" : "exec", action );


	button->cvarTest = "ui_adminFlags";
	button->enableCvar = "NeedsToBeSet";
	button->enableFlag = G_adminsys_cmd_flag(action);
	button->cvarFlags = CVAR_SHOW;

	if( Item_ValidateTooltipData(button) ) {
		button->toolTipData->text = g_adminsys_cmds[cmdid].function;
	}

	adminpanel_startItem( menu, button );
}

void adminpanel_addPlayerlist( menuDef_t *menu, float x, float y, float w, float h )
{
	itemDef_t *item;
	listBoxDef_t *listPtr;
	int maxchars;
	...
	if( w > 0 )
		maxchars = floor(((w-32) / 6));
	else
		maxchars = 0;

	maxchars -= 4; // clientnum room
	...
	// --
	item->window.foreColor[0] = item->window.foreColor[1] = item->window.foreColor[2] = .6f;
	item->window.foreColor[3] = 1.f;
	item->window.flags |= WINDOW_FORECOLORSET;
	item->window.outlineColor[0] = item->window.outlineColor[1] = item->window.outlineColor[2] = .5f;
	item->window.outlineColor[3] = .4f; // selected overlay
	item->window.border = WINDOW_BORDER_FULL;
	item->window.borderColor[0] = item->window.borderColor[1] = item->window.borderColor[2] = .5f; //.2f;
	item->window.borderColor[3] = .5f;

	item->special = FEEDER_PLAYER_LIST;

	Item_ValidateTypeData(item);
	if (!item->typeData)
		return;
	listPtr = (listBoxDef_t*)item->typeData;
	listPtr->elementStyle = LISTBOX_TEXT;
	listPtr->elementWidth = 200;
	listPtr->elementHeight = 12;

	listPtr->numColumns = 2;
	listPtr->columnInfo[0].pos = 0;
	listPtr->columnInfo[0].width = (w-32) - 16;
	listPtr->columnInfo[0].maxChars = maxchars; //33;

	// clientnum
	listPtr->columnInfo[1].pos = (w-32) - 8;
	listPtr->columnInfo[1].width = 16;
	listPtr->columnInfo[1].maxChars = 3;
...
void adminpanel_addText_special( menuDef_t *menu, float x, float y )
{
	itemDef_t *item;
	...
	item = adminpanel_addItem( menu );
		...
		item->type = ITEM_TYPE_TEXT;
	Item_ValidateTypeData(item);
		item->text = "ERROR"; // needs to be set
		item->font = UI_FONT_COURBD_30;
		item->textscale = .2f;
		item->textalignx = 3.f;
		item->textaligny = 12.f;
		item->window.foreColor[0] = item->window.foreColor[1] = item->window.foreColor[2] = 1.0f;
		item->window.foreColor[3] = 0.9f;
		item->window.flags |= WINDOW_FORECOLORSET;
		//item->window.border = WINDOW_BORDER_FULL;
		//item->window.borderColor[0] = item->window.borderColor[1] = item->window.borderColor[2] = .8f;
		...
		item->window.flags |= WINDOW_ADMINPANELHACK;
...
void adminpanel_Update( void )
{
	static char pname[MAX_NAME_LENGTH];
	int level = 0;

	DC->getCVarString( "name", pname, sizeof(pname) );

	level = DC->getCVarValue( "ui_adminLevel" );

	//Q_strncpyz( ui_adminsysPlayerName, pname, sizeof(ui_adminsysPlayerName) );
	//ui_adminsysLevel = level;

	message_hack = va( "^7Hello ^3%s^7, you are currently a level ^1%d^7 user.", pname, level );
}

qboolean adminpanel_Set(menuDef_t *menu)
{
	int temp;
	int y = 0;
	int x = 0;
	int xmove = BUTTONS_WIDTH + 6;

	//Q_strncpyz( menu->window.name, (const char *)"ingame_admin", sizeof("ingame_admin") );
	menu->window.name = "ingame_admin";
	menu->window.rect.x = (float)ADMINSYS_WINDOW_X;
	menu->window.rect.y = (float)(ADMINSYS_WINDOW_Y);
	menu->window.rect.w = (float)(ADMINSYS_WINDOW_WIDTH);
	menu->window.rect.h = (float)(ADMINSYS_WINDOW_HEIGHT);
	menu->window.style = WINDOW_STYLE_FILLED;
	menu->fullScreen = qfalse;
	menu->fadeClamp = 0.5;
	menu->fadeAmount = 0.075;
	//menu->onOpen = "\"setitemcolor\" \"background\" \"backcolor\" 0 0 0 0 ; \"fadein\" \"background\"";
	//menu->onOpen = "\"exec\" \"cg_draw2D 0\" ;";
	//menu->onESC = "\"close\" \"ingame_admin\" ; \"exec\" \"cg_draw2D 1\" ;";
	menu->onESC = "\"close\" \"ingame_admin\"";
	menu->window.flags |= WINDOW_VISIBLE;

// XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX

	adminpanel_addBackground( menu, 300 );

// ------------------------------------------------------------------------
	adminpanel_addWindow( menu, 0, 0, 
		ADMINSYS_WINDOW_WIDTH, ADMINSYS_WINDOW_HEIGHT, "ADMIN PANEL", 50 );

	// message window
	adminpanel_addWindow_blank( menu, 0, ADMINSYS_WINDOW_HEIGHT + 16, 
		ADMINSYS_WINDOW_WIDTH, (480 - (ADMINSYS_WINDOW_HEIGHT + 32) - 16) );

	// pinfo window
	adminpanel_addWindow_blank( menu, ((BUTTONS_WIDTH*2) + (6*3)), 76, 
		120+120+6, 100 );
// - - - - - - - - -
	y = 32;
	x = 6;

	//adminpanel_addText_small( menu, "test line here!!!", 6, 32 );
	adminpanel_addText_special( menu, x, y );
	y += 24;


	//adminpanel_addButton( menu, x, y, BUTTONS_WIDTH, BUTTONS_HEIGHT, "ADMINSTEST", .24, 11, "exec \"!admintest\";", "test tooltip" );
	adminpanel_addButton_adminsys( menu, x, y, BUTTONS_WIDTH, BUTTONS_HEIGHT, 
		"admintest me", .24, 11, "admintest", qfalse );

	adminpanel_addButton( menu, x + xmove, y, BUTTONS_WIDTH, BUTTONS_HEIGHT, "SHUFFLE RESTART", .24, 11, "exec \"cmd ref shuffleteamsxp\";", "" );


	y += 20;

	temp = (ADMINSYS_WINDOW_WIDTH - 12) - BUTTONS_WIDTH;
	adminpanel_addPlayerlist( menu, x, y, temp - BUTTONS_WIDTH - 12, 100 );
	//adminpanel_addButton( menu, temp + 6, y, BUTTONS_WIDTH, BUTTONS_HEIGHT, "admintest", .24, 11, "uiScript admintest", "" );

	y += 6;

	adminpanel_addButton_adminsys( menu, temp , y, BUTTONS_WIDTH, BUTTONS_HEIGHT, 
		"admintest", .24, 11, "admintest", qtrue );
	y += 18;
	adminpanel_addButton_adminsys( menu, temp, y, BUTTONS_WIDTH, BUTTONS_HEIGHT, 
		"mute", .24, 11, "mute", qtrue );
	y += 18;
	adminpanel_addButton_adminsys( menu, temp, y, BUTTONS_WIDTH, BUTTONS_HEIGHT, 
		"kick", .24, 11, "kick", qtrue );
	y += 18;
	adminpanel_addButton_adminsys( menu, temp, y, BUTTONS_WIDTH, BUTTONS_HEIGHT, 
		"ban", .24, 11, "ban", qtrue );
	y += 18;
	adminpanel_addButton_adminsys( menu, temp, y, BUTTONS_WIDTH, BUTTONS_HEIGHT, 
		"message", .24, 11, "message", qtrue );
	y += (22+6);

	adminpanel_addButton( menu, x, y, BUTTONS_WIDTH, BUTTONS_HEIGHT, "TEST", .24, 11, "", "" );


	return qtrue;
}

void adminpanel_Create( void )
{
	menuDef_t *menu = &Menus[menuCount];
