...
/*			if(1) {
				...
				buffer = va("%2d %s", n, namebuf);

				Q_strncpyz( namebuf, buffer, sizeof( namebuf ) );
			}*/
			Q_strncpyz( uiInfo.playerNames[uiInfo.playerCount], namebuf, sizeof( uiInfo.playerNames[0] ) );
			// codePRK adminpanel clientnums
			uiInfo.playerNumbers[uiInfo.playerCount] = n;
			...
			// codePRK adminpanel teamnums
			uiInfo.playerTeamNumbers[uiInfo.playerCount] = team2;
...
//			Menus_ActivateByName( "background_1", qtrue );
		...
		// codePRK adminpanel cmds
		else if (Q_stricmp(name, "!admintest") == 0) {
			if (uiInfo.playerIndex >= 0 && uiInfo.playerIndex < uiInfo.playerCount) {
				//trap_Cmd_ExecuteText( EXEC_APPEND, va("!admintest \"%s\"\n",uiInfo.playerNames[uiInfo.playerIndex]) );
				trap_Cmd_ExecuteText( EXEC_APPEND, va("!admintest \"%d\"\n",uiInfo.playerNumbers[uiInfo.playerIndex]) );

			}
		}
		else if (Q_stricmp(name, "!mute") == 0) {
			if (uiInfo.playerIndex >= 0 && uiInfo.playerIndex < uiInfo.playerCount) {
				if( !uiInfo.playerMuted[uiInfo.playerIndex] ) {
					//trap_Cmd_ExecuteText( EXEC_APPEND, va("!mute \"%s\"\n",uiInfo.playerNames[uiInfo.playerIndex]) );
					trap_Cmd_ExecuteText( EXEC_APPEND, va("!mute \"%d\"\n",uiInfo.playerNumbers[uiInfo.playerIndex]) );
				} else {
					trap_Cmd_ExecuteText( EXEC_APPEND, va("!unmute \"%d\"\n",uiInfo.playerNumbers[uiInfo.playerIndex]) );
				}
			}
		}
		else if (Q_stricmp(name, "!kick") == 0) {
			if (uiInfo.playerIndex >= 0 && uiInfo.playerIndex < uiInfo.playerCount) {
				//trap_Cmd_ExecuteText( EXEC_APPEND, va("!kick \"%s\"\n",uiInfo.playerNames[uiInfo.playerIndex]) );
				trap_Cmd_ExecuteText( EXEC_APPEND, va("!kick \"%d\"\n",uiInfo.playerNumbers[uiInfo.playerIndex]) );
			}
		}
		else if (Q_stricmp(name, "!ban") == 0) {
			if (uiInfo.playerIndex >= 0 && uiInfo.playerIndex < uiInfo.playerCount) {
				//trap_Cmd_ExecuteText( EXEC_APPEND, va("!kick \"%s\"\n",uiInfo.playerNames[uiInfo.playerIndex]) );
				trap_Cmd_ExecuteText( EXEC_APPEND, va("!ban \"%d\"\n",uiInfo.playerNumbers[uiInfo.playerIndex]) );
			}
		}
...
const char *UI_FeederItemText( float feederID, int index, int column, qhandle_t *handles, int *numhandles ) {
	...
	*numhandles = 0;
	if (feederID == FEEDER_HEADS) {
		...
		//}
		// codePRK adminpanel playerlist
		if (index >= 0 && index < uiInfo.playerCount) {
			if( column == 1 ) {
				static char s[4] = { "" };
				Com_sprintf( s, sizeof(s), "%2d", uiInfo.playerNumbers[index] );
				return s;
			}
			//if( column == 0 ) {
			else {
