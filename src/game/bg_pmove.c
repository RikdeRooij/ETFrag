...
#define PM_JUMP_STAMINA (cgs.realism & REAL_JUMP_STAMINA) 
//#define PM_TRACEHEAD (cgs.misc & MISC_TRACEHEAD)
// codePRK objective pickup
#define PM_WEAPONS cgs.weapons
...
#define PM_JUMP_STAMINA (g_realism.integer & REAL_JUMP_STAMINA)
//#define PM_TRACEHEAD (g_misc.integer & MISC_TRACEHEAD)
// codePRK objective pickup
#define PM_WEAPONS g_weapons.integer
#endif
...
static float PM_CmdScale( usercmd_t *cmd ) {
	...
	if ((pm->ps->weapon == WP_PANZERFAUST) ||
		...
		if( pm->skill[SK_HEAVY_WEAPONS] >= 3 || panzerwar > 0 ) {
			scale *= 0.75;
		} else {
			scale *= 0.5;
		}
...
#ifdef GS_BREAKLEGS
static qboolean PM_CheckLegsBroken (void)
{
	
	if(pm->ps->pm_type != PM_LEGSBROKEN) return qfalse;
	pm->ps->pm_type = PM_NORMAL;

	if( !(pm->ps->eFlags & EF_LEGSBROKEN) ) {
...
// codePRK objective pickup
	...
	if( PM_WEAPONS & WPF_WEAP_OBJ ) // codePRK no weapon with objective
	{
		int oldweapon = pm->ps->weapon;
//		gitem_t		*item = NULL;

		if ( (pm->ps->powerups[PW_REDFLAG]) || (pm->ps->powerups[PW_BLUEFLAG]) ) {
			if ( pm->ps->weaponstate != WEAPON_HOLSTER_IN ) {
			...
			//pm->ps->weaponTime = 500; //BG_Weapons[pm->ps->weapon].weapAnimations[PM_DropAnimForWeapon(pm->ps->weapon)].time;

			pm->ps->weapon = WP_NONE; // or add somthing visual?
			pm->ps->weapon = pm->ps->item;
			...
			//pm->ps->weaponTime = 500;
			if(oldweapon)
				pm->ps->weapon = oldweapon;

			PM_StartWeaponAnim(PM_RaiseAnimForWeapon(pm->ps->weapon));
			//pm->ps->weaponTime = 500; //BG_Weapons[pm->ps->weapon].weapAnimations[PM_RaiseAnimForWeapon(pm->ps->weapon)].time;
			return;
...
//				&& PM_UW_PLIERS)
	...
	// bacon - single fire
	if ( (pm->ps->pm_flags & PMF_FIREHELD) && ( 
		IS_SINGLEFIRE_WEAPON( pm->ps->weapon ) ) ) {
		return;
	}
...
//				else 
				if(CG_WeaponSelectable(WP_STEN))
