...
qboolean G_tryObjDrop( gentity_t *ent, qboolean knifeonly )
{
	if( ent->client->ps.powerups[PW_REDFLAG] ||
		ent->client->ps.powerups[PW_BLUEFLAG] ) {
			int maxdrops = (g_dropObj.integer - 1);
			int drops = ent->client->pmext.objDrops;

			if( knifeonly && (ent->client->ps.weapon != WP_KNIFE && ent->client->ps.weapon != WP_NONE) ) {
				CP("cp \"You must switch to knife to drop objective\" 1");
				return qfalse;
			}

			if ( level.time - ent->client->dropWeaponTime < 2000 ) // dont allow as often
				return qfalse;
			...
			if( (maxdrops < 0) || (drops <= maxdrops ) ) {
				...
				if( (maxdrops - drops) > 1 ) {
					CP( va( "cp \"%i drops left!\" 1", (maxdrops - drops) ) );
					return qtrue;
				} else if( (maxdrops - drops) > 0 ) {
					// last drop
					CP("cp \"^1only 1 drop left!\" 1");
					return qtrue;
				} else {
					// that was the last drop
					CP("cp \"^1NO drops left!\" 1");
					return qtrue;
				}
			} else {
				// all other tries after last drop
				CP("cp \"^1NO drops left!\" 1");
				return qfalse;
			}
	}
...
if( !G_tryObjDrop( ent, qfalse ) ) {
// we didn't drop the obj, so drop out weapon

	if ( level.time - ent->client->dropWeaponTime < 500 ) // dont allow as often
		return;

	//primary = G_GetPrimaryWeaponForClient( ent->client );
	primary = G_GetPrimaryWeaponForClient2Drop( ent->client );
