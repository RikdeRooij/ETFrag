...
char *G_createStats(gentity_t *refEnt, gentity_t *reqEnt)
{
	...
	if(dwWeaponMask != 0) {
		// Dens: only show seperated teamdamge to newer clients (logging too of course)
			...
			Q_strcat(strWeapInfo, sizeof(strWeapInfo), va(" %d",
...
{
	int i, j, cnt, eff;
	int tot_kills, tot_deaths, tot_gp, tot_gibs, tot_sui, tot_tk, tot_dg, tot_dr, tot_tdg, tot_tdr;
	gclient_t *cl;
	char *ref;
	...
	for(i=TEAM_AXIS; i<=TEAM_ALLIES; i++) {
		...
		CP("sc \"\n^7TEAM   Player          Kll Dth Sui TK Eff ^3Gib^7    ^2DG    ^1DR  ^6TDG  ^4TDR  ^3Score\n"
			"^7--------------------------------------------------------------------------\n\"");
