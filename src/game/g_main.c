...
vmCvar_t g_adminsysDir;
vmCvar_t g_adminsysPrefix;

vmCvar_t g_kickTime;

vmCvar_t g_airstrikeRestoreValue;

vmCvar_t g_noFallDamage;
...
// unlagged server options
	...
	// Dens: settings file
	{ &g_bannersFile, "g_bannersFile", "", 0, 0 },
...
// codePRK CVARs server
	...
	{ &g_hardcore, "g_hardcore", "0", 0},


	{ &g_hitSounds, "g_hitSounds", "0", 0 },

	{ &g_turrets, "g_turrets", "1", 0 },

	{ &g_adminsysDir, "g_adminsysDir", "adminsys", 0, 0},
	{ &g_adminsysPrefix, "g_adminsysPrefix", "!", CVAR_ROM, 0},

	{ &g_kickTime, "g_kickTime", "120", 0, 0},

	{ &g_airstrikeRestoreValue, "g_airstrikeRestoreValue", "0", 0, 0},

	{ &g_noFallDamage, "g_noFallDamage", "0", 0, 0},

#ifdef GS_DEBUG
	{ &g_testVar, "g_testVar", "0", 0, 0},
#endif

// --

	{ NULL, "mod_version", MOD_VERSION, CVAR_SERVERINFO | CVAR_ROM },
	{ NULL, "mod_url", MOD_URL, CVAR_SERVERINFO | CVAR_ROM },
...
qboolean G_IsEtfraginfoCvar(vmCvar_t *c)
{
	return (
		// codePRK
		c == &g_realism ||
...
void G_UpdateEtfraginfo(void)
{
	char cs[MAX_INFO_STRING];
	cs[0] = '\0';

	Info_SetValueForKey(cs, "etfrag", MOD_VERSION);

	Info_SetValueForKey(cs, "g_realism",
		va("%i", g_realism.integer));
	Info_SetValueForKey(cs, "g_hardcore",
		va("%i", g_hardcore.integer));

	// codePRK adminpanel
	Info_SetValueForKey(cs, "g_adminsysDir",
		va("%s", g_adminsysDir.string));
	...
	trap_SetConfigstring(CS_ETFRAGINFO, cs);
}
...
void G_PanzerWar()
{
	...
	if( g_panzerwar.integer && !panzerwar_enabled ) {
		...
		trap_Cvar_Set( va( "%s_panzerwar", GAMEVERSION ),
			va( "%i %i %i", panzer->maxammo, panzer->nextShotTime,
				panzer->fireDelayTime ) );
		...
		sscanf( buffer, "%i %i %i", &panzer->maxammo,
			&panzer->nextShotTime, &panzer->fireDelayTime );
...
void G_SniperWar()
{
	...
	if( g_sniperwar.integer && !sniperwar_enabled ) {
		...
		trap_Cvar_Set( va( "%s_sniperwar", GAMEVERSION ),
			va( "%i %i", garand->maxammo, k43->maxammo ) );
		...
		trap_Cvar_VariableStringBuffer( va( "%s_sniperwar", GAMEVERSION ),
			buffer, sizeof( buffer ) );
		sscanf( buffer, "%i %i", &garand->maxammo, &k43->maxammo );
	...
	for ( i = 0, cv = gameCvarTable ; i < gameCvarTableSize ; i++, cv++ ) {
		if ( cv->vmCvar ) {
			...
			if(cv->modificationCount != cv->vmCvar->modificationCount) {
				...
				else if(G_IsEtfraginfoCvar(cv->vmCvar)) {
					G_UpdateEtfraginfo();
				}
...
//		BotAILoadMap( restart );
	...
	if ( g_adminsysDir.string[0] ) {
		G_adminsystem_Init(NULL, -1);
	}
#endif


#ifdef GS_NEWGAMETYPE
	G_Printf( "^6### %s: %i\n", level.rawmapname, level.DMSpawnPoints );
#endif
}
...
#endif
#ifdef GS_ADMINSYS
	G_adminsys_cleanup_all();
#endif
	G_xpsave_cleanup();


	level.DMSpawnPoints = 0; 
}
...
void CheckIntermissionExit( void ) {
	...
	for( i = 0; i < level.numConnectedClients; i++ ) {
		...
		// corresponding change on the client behavior, so that visual
...
*/
		...
		if(g_gamestate.integer == GS_WARMUP_COUNTDOWN) {
			if(level.time > level.warmupTime) {
				...
				G_globalSound("sound/etfrag/osp_fight.wav");
...
void CheckVote( qboolean forced ) {
	...
	for( i = 0; i < level.numConnectedClients; i++ ) {
		loopent = g_entities + level.sortedClients[i];
		//if(loopent->client->sess.referee == RL_REFEREE){
			trap_SendServerCommand( loopent-g_entities, va("cpm \"^2Axis vote results, yes: %i, no %i\n\"",
				level.voteInfo.axisVoteYes, level.voteInfo.axisVoteNo));
			trap_SendServerCommand( loopent-g_entities, va("cpm \"^2Allies vote results, yes: %i, no %i\n\"",
				level.voteInfo.alliesVoteYes, level.voteInfo.alliesVoteNo));
...
int G_highest_multikill(gentity_t *ent)
{
	...
	return highest;*/
	return ent->client->pers.multikill_count;
}
...
void G_check_multikill(gentity_t *ent, int number)
{
	...
	switch( number ) {
		case 2: AP(va("ks \"^1Double Kill! \n%s\"", mkmsg )); break;
		case 3: AP(va("ks \"^1Triple kill! \n%s\"", mkmsg )); break;
		case 4: AP(va("ks \"^1Multikill! \n%s\"", mkmsg )); break;
		case 5: AP(va("ks \"^1Megakill! \n%s\"", mkmsg )); break;
		default: return;
	}
...
void etfrag_version( gentity_t *ent )
{
	const char *s = MOD_VERSION;
	//int i = 0;
	...
	G_refPrintf( ent, " ^7ETFrag ^1: ^7%-12s %s %s\n",
		va( "%s", s ),
		CPUSTRING, __DATE__ );
}

// codePRK
void G_RailTrail( int clientnum, vec3_t start, vec3_t end, vec3_t color, int lifetime )
{
	gentity_t *tent;

	tent = G_TempEntity( start, EV_RAILTRAIL );
	VectorCopy( end, tent->s.origin2 );
	tent->s.dmgFlags = -1; // line
	tent->s.eventParms[0] = color[0];
	tent->s.eventParms[1] = color[1];
	tent->s.eventParms[2] = color[2];
	tent->s.effect1Time = lifetime;
	tent->s.otherEntityNum2 = clientnum;
