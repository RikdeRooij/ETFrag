...
void SelectTESTSpawnPoint ( vec3_t origin, vec3_t angles ) {
	vec3_t		point;
	float		groundh;
	float		mapw;
	float 		maph;
	...
	vec3_t axis[3];
	int spawnspace = 256;
	int spawnheighttest = 32;
	int tries = 0;
	int maxtries = 1024;
	int OKdirections = 0;

	mapw = (level.mapcoordsMaxs[0] - level.mapcoordsMins[0]);
	maph = (level.mapcoordsMaxs[1] - level.mapcoordsMins[1]);

	VectorCopy( vec3_origin, point );

	while( 1 ) {
		if( (point[2] <= -MAX_MAP_SIZE) || (point[2] >= MAX_MAP_SIZE) ) { // last place we want to spawn...
			point[2] = 64; // most maps have the floor around 0 ? so this would be my best guess
		}

		tries++;
		if( tries > maxtries ) {
			G_Printf( "^1ETFrag: ERROR Spawning Player: unsafe\n" );
			// we did not find a spawnpoint yet, just break and use last, and hope for the best
			break;
		}

		point[0] = (level.mapcoordsMins[0] + (random() * mapw) );
		point[1] = (level.mapcoordsMins[1] + (random() * maph) );
		point[2] = MAX_MAP_SIZE;
		groundh = BG_GetGroundHeightAtPoint(point);
		if( (groundh <= -MAX_MAP_SIZE) || (groundh >= MAX_MAP_SIZE) ) {
			continue;
		}
		groundh += spawnheighttest;
		point[2] = groundh;
		VectorCopy( point, trPoint );
		// we are not into smthing solid
		trap_Trace( &tr, point, playerMins, playerMaxs, trPoint, -1, MASK_PLAYERSOLID);
		//G_RailTrail( 0, point, trPoint, colorWhite, 10000 );
		if( tr.startsolid ) {
			continue;
		}
		...
		// we really have ground here
		VectorMA( point, (-spawnheighttest*2), axis[2], trPoint );
		trap_Trace( &tr, point, playerMins, playerMaxs, trPoint, -1, MASK_SOLID);
		//G_RailTrail( 0, point, trPoint, colorRed, 10000 );
		if( tr.fraction == 1.0f ) {
			continue;
		}

		// we can stand here
		VectorMA( point, 48, axis[2], trPoint );
		trap_Trace( &tr, point, playerMins, playerMaxs, trPoint, -1, MASK_PLAYERSOLID);
		//G_RailTrail( 0, point, trPoint, colorBlue, 10000 );
		if( tr.fraction != 1.0f ) {
			continue;
		}

		// check around for solids
		OKdirections = 4;
		VectorMA( point, spawnspace, axis[1], trPoint );
		trap_Trace( &tr, point, playerMins, playerMaxs, trPoint, -1, MASK_PLAYERSOLID);
		//G_RailTrail( 0, point, trPoint, colorRed, 10000 );
		if( tr.fraction != 1.0f ) {
			OKdirections--;
			angles[YAW] = 270; // turn away from there
		}
		VectorMA( point, -spawnspace, axis[1], trPoint );
		trap_Trace( &tr, point, playerMins, playerMaxs, trPoint, -1, MASK_PLAYERSOLID);
		//G_RailTrail( 0, point, trPoint, colorGreen, 10000 );
		if( tr.fraction != 1.0f ) {
			OKdirections--;
			angles[YAW] = 90;
		}
		VectorMA( point, spawnspace, axis[0], trPoint );
		trap_Trace( &tr, point, playerMins, playerMaxs, trPoint, -1, MASK_PLAYERSOLID);
		//G_RailTrail( 0, point, trPoint, colorBlue, 10000 );
		if( tr.fraction != 1.0f ) {
			OKdirections--;
		...
		VectorMA( point, -spawnspace, axis[0], trPoint );
		trap_Trace( &tr, point, playerMins, playerMaxs, trPoint, -1, MASK_PLAYERSOLID);
		//G_RailTrail( 0, point, trPoint, colorYellow, 10000 );
		if( tr.fraction != 1.0f ) {
			OKdirections--;
			angles[YAW] = 0;
		}
		...
		if( (OKdirections == 4) && (tries < maxtries/2) ) {
			// we're not near a wall, we prob don't like that
			continue;
		}
	...
	G_Printf( "^2ETFrag: Spawning Client! (Ground: \"%f\")\n", point[2] );

	VectorCopy (point, origin);
	origin[2] -= spawnheighttest;
	origin[2] += 9;
	origin[2] += 8;
	//VectorCopy (vec3_origin, angles); moved up
}
#endif
