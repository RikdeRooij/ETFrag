...
void BodySink2( gentity_t *ent ) {
	...
	// codePRK bodysink slower
	//VectorSet( ent->s.pos.trDelta, 0, 0, -8 );
	VectorSet( ent->s.pos.trDelta, 0, 0, -2 );
}
...
#ifdef GS_PRIVATEBETA

qboolean G_AllowedPrivateBeta(char *guid, char *name)
{
	int handle;
	pc_token_t token;
	char* srcFilename;
	...
	srcFilename = "Private.cfg";


	handle = trap_PC_LoadSource( srcFilename );
	if (!handle) {
		//G_Printf("^1ERROR loading: '%s'\n", srcFilename);
		G_Printf("^1ERROR loading Private.cfg, no server entry allowed\n");
		return qfalse;
	}
	...
	while( 1 ) {
		...
		strguid = token.string;
		//Q_strcat( strguid, sizeof(strguid), va( "%s", token.string ) );

		//G_Printf("^6%s : %s\n",guid, strguid);
		if(!Q_stricmp( guid, strguid )) {
			// get name
			if( trap_PC_ReadToken( handle, &token ) ) {
				if(!Q_stricmp( token.string, "=" )) {
					if ( trap_PC_ReadToken( handle, &token ) ) {
						strname = token.string;
					}
				}
			}

			trap_PC_FreeSource( handle );
			//G_Printf("^3PrivateBeta:^2 Access Granted ^7(user: %s^7)\n", strname);
			AP( va("print \"^3PrivateBeta:^2 Access Granted ^7%s^7!%s\n\"", name, strname ? va(" ^7(%s^7)",strname) : "") );
			return qtrue;
		}
	...
	//G_Printf("^3PrivateBeta: ^1Access Denied ^7(player: %s^7)\n", name);
	AP( va("print \"^3PrivateBeta: ^1Access Denied ^7(player: %s^7, guid: %s)\n\"", name, guid) );
...
//	}
	...
	if ( ent->r.svFlags & SVF_BOT ) {
		...
		s = va( "n\\%s\\t\\%i\\c\\%i\\r\\%i\\m\\%s\\s\\%s\\dn\\%s\\dr\\%i\\w\\%i\\lw\\%i\\sw\\%i\\mu\\%i\\ref\\%i\\uci\\%u\\lc\\%i\\tv\\%i\\sc\\%i%s",
			client->pers.netname, 
			client->sess.sessionTeam, 
			client->sess.playerType, 
			client->sess.rank, 
...
/*	if(1) {
		char *uuidStr;
		uuidStr = va( "uuid\\%s", client->sess.uuid, 
		trap_SetConfigstring( CS_UUID + clientNum, uuidStr );
	}*/
...
char *ClientConnect( int clientNum, qboolean firstTime, qboolean isBot ) {
	...
	if (!isBot && !(ent->r.svFlags & SVF_BOT))
	{
		...
		if (conn_per_ip > g_maxConnsPerIP.integer) {
			G_LogPrintf("rejecting client from %s "
...
#ifdef GS_PRIVATEBETA
	if( !G_AllowedPrivateBeta( guid, name ) ) {
		trap_DropClient( clientNum, "^3PrivateBeta: ^1Access Denied ^7(Only beta testers are allowed)\n", 0 );
	}
#endif
...
/*			spawnPoint = NULL;
			...
			if( g_gametype.integer == GT_WOLF_DM ) {
				spawnPoint = NULL;
				SelectTESTSpawnPoint( spawn_origin, spawn_angles );
			} else {
#endif
...
#ifdef GS_CCHARACTERS
		if( client->sess.gcbody != client->sess.glcbody )
			update = qtrue;

		client->sess.gcbody = client->sess.glcbody;
#endif
...
#ifdef GS_BREAKLEGS
		else if(hitEnt->client->ps.eFlags & EF_LEGSBROKEN) {
			return 14;
		}
#endif
