...
//#define GS_PRIVATEBETA	// strict access

#define GS_DEBUG		// debug
#define GS_REFUSEABLE	// refusable dynos
#define GS_TURRETS		// deployable turrets
#define GS_ADMINSYS		// admin system
#define GS_CCHARACTERS	// custom characters
#define GS_NEWGAMETYPE	// new gametype test

#define GS_BREAKLEGS		// TODO: no walking when fallen from great hight


#define GS_UUID			// unique user id
...
// ETPUB (allow to temp re-enable)
//#define GS_OMNIBOT		// MOVED TO COMMANDLINE
//#define GS_SHRUBBOT

// CMDLINE: #define NO_BOT_SUPPORT
...
#define PMF_FIREHELD		1024	// bacon - single fire
...
//	PERS_WOLFKICK
    ...
    PERS_HEADSHOTS,					// hitsounds
    PERS_KILLSPREE,					// killing spree as a number
...
#ifdef GS_BREAKLEGS
#define EF_LEGSBROKEN		0x00800000		// codePRK broken legs
#endif
...
#define REAL_JUMP_STAMINA	1	// jumping needs stamina
...
#define WPF_WEAP_OBJ				1		// objective pickup
#define WPF_WEAP_PICKUP				2		// pickup any


#define MISC_SHOW_KILLERHP 1 // Dens: show the hp of your attacker when you got killed
...
typedef enum {
    SK_BATTLE_SENSE,
    SK_EXPLOSIVES_AND_CONSTRUCTION,					// engi
    SK_FIRST_AID,									// medic
    SK_SIGNALS,										// fieldops
    SK_LIGHT_WEAPONS,
    SK_HEAVY_WEAPONS,								// soldier
    SK_MILITARY_INTELLIGENCE_AND_SCOPED_WEAPONS,	// covertops
    SK_NUM_SKILLS
} skillType_t;
...
#ifdef GS_CCHARACTERS
    qhandle_t			customSkins[MAX_CUSTOMSKINS];
#endif
...
#define IS_SINGLEFIRE_WEAPON(w) \
    (	w == WP_K43				|| w == WP_K43_SCOPE				|| \
        w == WP_GARAND			|| w == WP_GARAND_SCOPE				|| \
        w == WP_FG42SCOPE		|| \
        w == WP_LUGER			|| w == WP_SILENCER					|| \
        w == WP_COLT			|| w == WP_SILENCED_COLT			|| \
        w == WP_AKIMBO_COLT		|| w == WP_AKIMBO_SILENCEDCOLT		|| \
        w == WP_AKIMBO_LUGER	|| w == WP_AKIMBO_SILENCEDLUGER		|| \
...
#define IS_PRIMARY_WEAPON(w) \
    (	IS_ENGINEER_WEAPON(w)		|| \
        IS_HEAVY_WEAPON(w)			|| \
        IS_COVERT_WEAPON(w)			)

#define IS_BULLET_WEAPON(w)		\
    (	IS_SUBMACHINEGUN(w)		|| \
        IS_COVERT_WEAPON(w)		|| \
        IS_SECONDARY(w)			|| \
        IS_SCOPED_WEAPON(w)		|| \
