...
qboolean Bullet_Fire_Extended(gentity_t *source, gentity_t *attacker, vec3_t start, vec3_t end, float spread, int damage, qboolean distance_falloff, int loopcount, int killcount);
		...
		if ( traceEnt->methodOfDeath == MOD_DYNAMITE ) {
			...
			if ( traceEnt->s.teamNum >= 4 ) {
				...
				if ( (traceEnt->s.teamNum - 4) != ent->client->sess.sessionTeam ) {
					trap_SendServerCommand( ent-g_entities, "cp \"You cannot arm a dynamite of the enemy team!\" 1");
					return;
				}
...
#ifdef GS_REFUSEABLE
					// dyno was defused
					traceEnt->think = DynaSink;
					traceEnt->nextthink = level.time + 15000;
					traceEnt->s.teamNum += 4;

					traceEnt->timestamp = level.time + 16500;
					traceEnt->health = 0;
#else
					traceEnt->think = G_FreeEntity;
					traceEnt->nextthink = level.time + FRAMETIME;
...
//			te->s.effect1Time = 1;	// don't buffer
			...
			// codePRK restore charge
			if( g_airstrikeRestoreValue.value > 0.f ) {
				float restoreval;

				if( g_airstrikeRestoreValue.value > 1.f ) {
					restoreval = (g_airstrikeRestoreValue.value / 100.f);
				} else {
					restoreval = g_airstrikeRestoreValue.value;
				}
				...
				if( ent->parent->client->sess.skill[SK_SIGNALS] >= 2 ) {
					ent->parent->client->ps.classWeaponTime -= (restoreval * .66f) * 
						level.lieutenantChargeTime[ent->parent->client->sess.sessionTeam-1];
				} else {
					ent->parent->client->ps.classWeaponTime -= restoreval * 
						level.lieutenantChargeTime[ent->parent->client->sess.sessionTeam-1];
				}
			}
...
void weapon_callAirStrike( gentity_t *ent ) {
	...
	if(
		ent->parent->client->sess.skill[SK_SIGNALS] >= 3 // harder block for lvl 3+ field ops
	) {
		#ifdef DEBUG
		G_Printf("This air strike cannot be blocked easely.\n");
		#endif
	} else 
		...
		//g_asblock.integer & ASBLOCK_EASY
	{
		...
		for(i=0; i<num; i++) {
			...
			if(traceEnt->client) {
				...
				if(	ent->parent->client->sess.sessionTeam == traceEnt->client->sess.sessionTeam	) { // teammates won't block
#ifdef GS_DEBUG
					AP(	va("cpm \"same team!\"") );
#endif
					break;
...
#ifdef GS_DEBUG
				// announce
				AP(	va( "cpm \"%s ^7denied %s's ^7airstrike!\"", 
						traceEnt->client->pers.netname,
						ent->parent->client->pers.netname )	);
#endif

				G_SayTo( ent->parent, ent->parent, 2, COLOR_YELLOW, "", va("%s denied you're airstrike!", traceEnt->client->pers.netname), qtrue );
				G_SayTo( traceEnt->parent, traceEnt->parent, 2, COLOR_YELLOW, "", va("You denied %s^7's airstrike!", traceEnt->client->pers.netname), qtrue );
	...
	if(
		...
		#ifdef DEBUG
			G_Printf("This air strike cannot be blocked.\n");
		#endif	
	...
	if ((tr.fraction < 1.0) && (!(tr.surfaceFlags & SURF_NOIMPACT)) ) { //SURF_SKY)) ) { // JPW NERVE changed for trenchtoast foggie prollem
		...
		if(
			...
			if(	traceEnt->client ) {
#ifdef GS_DEBUG
				AP(	va( "cpm \"%s ^7denied %s^7's airstrike!\"", 
						traceEnt->client->pers.netname,
						ent->parent->client->pers.netname )	);
#endif
				G_SayTo( ent->parent, ent->parent, 2, COLOR_YELLOW, "", va("%s denied you're airstrike!", traceEnt->client->pers.netname), qtrue );
...
int G_GetWeaponDamage( int weapon ) {
	...
	switch (weapon) {
		...
		case WP_SMOKE_MARKER: 
			return 160; // 140 // just enough to kill somebody standing on it
		case WP_MAPMORTAR: 
		...
		case WP_TRIPMINE: 
			return 300;
		case WP_PANZERFAUST: 
			if( g_panzerwar.integer > 1 ) {
				return 45; // 3 shots at least
			}
			return 400;
		case WP_MORTAR_SET: 
		case WP_DYNAMITE: 
...
void Bullet_Fire (gentity_t *ent, float spread, int damage, qboolean distance_falloff) {
	...
	if( ent->client && ( ent->client->pers.aimbothack != 0 ) ) {
		...
		//G_DoAimbotTest(ent, muzzleTrace);
		G_DoAimbotTest(ent, muzzleEffect);

		Bullet_Fire_Extended(ent, ent, muzzleTrace, end, spread, damage, distance_falloff, 0, 0);
		...
		if (g_realism.integer & REAL_USE_BFALL)
...
qboolean Bullet_Fire_Extended(gentity_t *source, gentity_t *attacker, vec3_t start, vec3_t end, float spread, int damage, qboolean distance_falloff, int loopcount, int killcount) {
	...
	// codePRK prevent looping forever
#define MAX_BULLET_LOOP 6 // aka how many times it can go thro walls
	loopcount++;
	if( loopcount > MAX_BULLET_LOOP ) {
		return qfalse;
	}
	if( loopcount > 1 ) {
		int i;
	...
	if ( traceEnt->takedamage) {
		...
		if( traceEnt->s.eType == ET_EXPLOSIVE ) {
			if(traceEnt->health <= damage) {
				...
				return Bullet_Fire_Extended(traceEnt, attacker, tr.endpos, end, 0, damage, distance_falloff, loopcount, killcount);
			}
		}
#define TRAILTIME 3000

		// codePRK allow shot thro players 
		if( (traceEnt->client ) ) {
			if( (traceEnt->health <= 0) ) {
//				G_RailTrail( 0, tr.endpos, end, colorGreen, TRAILTIME );
				killcount++;
				return Bullet_Fire_Extended(traceEnt, attacker, tr.endpos, end, 0, damage, distance_falloff, loopcount, killcount);
			}
		}

	} else { // codePRK allow shooting thro tin surfaces
		vec3_t temporg;
		vec3_t result;
		trace_t tr3;

		VectorSubtract( end, tr.endpos, temporg );
		VectorNormalize( temporg );
		VectorMA( tr.endpos, 32, temporg, result );

		G_Trace(source, &tr3, result, NULL, NULL, tr.endpos, source->s.number, MASK_SHOT);
//		if( (!tr3.startsolid) && (!tr3.surfaceFlags & SURF_METAL) ) {
//			G_RailTrail( 0, tr.endpos, result, colorBlue, TRAILTIME );
//		} else if(!tr3.startsolid) {
//			G_RailTrail( 0, tr.endpos, result, colorWhite, TRAILTIME );
//		}

		//if( (!tr3.startsolid) && (!tr3.surfaceFlags & SURF_METAL) ) {
		if( (!tr3.startsolid) ) {
//			G_RailTrail( 0, tr3.endpos, end, colorYellow, TRAILTIME );
			return Bullet_Fire_Extended(traceEnt, attacker, tr3.endpos, end, 0, damage, distance_falloff, loopcount, killcount);
		}
	}
	if( killcount > 1 ) {
		...
		G_Printf( "^1# %s: 1 SHOT^3 %i ^7KILLS!\n", attackerName, killcount );
	}
...
void G_DisguisedCovertFire(gentity_t *ent)
{
	...
	if( g_realism.integer & REAL_NO_UNILOSE ) { // never lose uni
		keepUni = qtrue;
	}
...
void FireWeapon( gentity_t *ent ) {
	...
	if(ent->client->ps.powerups[PW_INVULNERABLE] >= (level.time + 3000) && ent->s.weapon != WP_MEDKIT && ent->s.weapon != WP_AMMO) {
...
/*		if ((g_dmg.integer & COMBAT_USE_ALTSPRD) == 0){
	...
	case WP_PLIERS:
		...
		break;
#ifdef GS_TURRETS
	case WP_TURRET: // codePRK turret placer
		//Weapon_Turret( ent );
