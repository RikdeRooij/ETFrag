...
#ifdef GS_BREAKLEGS
	// codePRK hack, broken legs
	if( (ps->eFlags & EF_LEGSBROKEN) ) {

		if( movetype != ANIM_MT_FALLEN && movetype != ANIM_MT_FLAILING )
			return -1;

		if( movetype == ANIM_MT_FALLEN || movetype == ANIM_MT_FLAILING ) {
			scriptCommand->bodyPart[0] = ANIM_BP_LEGS;
			scriptCommand->bodyPart[1] = ANIM_BP_UNUSED;
		}
		else {
			if( scriptCommand->bodyPart[0] == ANIM_BP_BOTH ) {
				scriptCommand->bodyPart[0] = ANIM_BP_TORSO;
				scriptCommand->bodyPart[1] = ANIM_BP_UNUSED;
			} else if( scriptCommand->bodyPart[0] == ANIM_BP_TORSO ) {
				scriptCommand->bodyPart[0] = ANIM_BP_TORSO;
				scriptCommand->bodyPart[1] = ANIM_BP_UNUSED;
			} else if( scriptCommand->bodyPart[0] == ANIM_BP_LEGS && scriptCommand->bodyPart[1] == ANIM_BP_TORSO ) {
				scriptCommand->bodyPart[0] = ANIM_BP_UNUSED;
				scriptCommand->bodyPart[1] = ANIM_BP_TORSO;
			} else {
				return -1;
			}
		}
