	...
	if (ent->s.onFireEnd && ent->client) {
		if (level.time - ent->client->lastBurnTime >= MIN_BURN_INTERVAL) { 
			...
			if ((ent->s.onFireEnd > level.time) && (ent->health > 0)) {
				...
				G_Damage (ent, attacker, attacker, NULL, NULL, 6, DAMAGE_NO_KNOCKBACK, MOD_FLAMETHROWER); // JPW NERVE was 7
			}
		}
	}
	// jpw
		...
		if( ( g_inactivityOptions.integer & IO_ETMAIN_BEHAVIOR ) ||
			...
			if( client->inactivityWarning &&
				...
				if( client->sess.sessionTeam != TEAM_SPECTATOR ) {
					if( 
#ifdef GS_SHRUBBOT
						!G_shrubbot_permission( ent, SBF_ACTIVITY ) ||
#endif
...
void ClientTimerActions( gentity_t *ent, int msec ) {
	...
	while( client->timeResidual >= 1000 ) {
		...
		regenRate1 = 3; // default
		regenRate2 = 2; // default

		if( g_realism.integer & REAL_SLOW_REGEN ) {
			regenRate1 = 1;
			regenRate2 = 0;
		}
...
void G_FallDamage(gentity_t *ent, int event)
{
	...
	// do goomba damage
	//goombaDmg = damage * 10; // minimum: 50
...
void ClientEvents( gentity_t *ent, int oldEventSequence ) {
	...
	for ( i = oldEventSequence ; i < client->ps.eventSequence ; i++ ) {
		...
		case EV_FIRE_WEAPON_MG42:
			...
			if( !( g_realism.integer & REAL_NO_UNILOSE ) ) {
...
//unlagged - true ping
	...
	if(client->warping && g_maxWarp.integer && !G_DoAntiwarp(ent)) {
		...
		int maxWarp = g_maxWarp.integer; // codePRK

		if( maxWarp < 3 ) {
			maxWarp = 3; // limiting this ffs
		}
...
#ifdef GS_BREAKLEGS
	else if(client->ps.pm_type == PM_LEGSBROKEN) {
		// handled in pmove (PM_CheckLegsBroken)
	}
#endif
	else {
		if( !client->frozen ) {
...
#define SPEED_START 64
#define SPEED_BOTTOM 66
	if( (g_realism.integer & REAL_HEALTH_SPEED)
		&& ent->health > 0){
			if(ent->health < (int)(client->ps.stats[STAT_MAX_HEALTH] * 
				SPEED_START / 100)){
					client->ps.speed = (int)(((ent->health * 
						(100.000f - SPEED_BOTTOM)/
						(SPEED_START * client->ps.stats[STAT_MAX_HEALTH])) +
						SPEED_BOTTOM / 100.000f) * g_speed.value);
			}	
	}
	...
	// pheno: don't allow frozen players to use +attack
...
void ClientEndFrame( gentity_t *ent ) {
	...
	if( ent->client && ( ent->client->pers.aimbothack ) ) {
		G_DoAimbotTest(ent, vec3_origin);
	}
#endif
...
#ifdef GS_TURRETS
	// codePRK toggle draw
	if( (ent->s.weapon == WP_TURRET) && !ent->AASblocking  )
		Cmd_TestTurret_f( ent );
	else if( (ent->s.weapon != WP_TURRET) && ent->AASblocking  )
		Cmd_TestTurret_f( ent );
#endif
...
//unlagged - smooth clients #1
	...
	// codePRK && frames > 3
	if(g_maxWarp.integer && frames > g_maxWarp.integer && frames > 3 && !G_DoAntiwarp(ent)) 
...
//unlagged - smooth clients #1
	...
	if(1) {
		...
		VectorCopy(base->r.currentOrigin, b1);
		VectorCopy(base->r.currentOrigin, b2);
		VectorAdd(b1, base->r.mins, b1);
		VectorCopy(base->r.maxs, maxs);
		maxs[2] = 48;
		VectorAdd(b2, maxs, b2);
		bboxEnt = G_TempEntity( b1, EV_RAILTRAIL );
		VectorCopy(b2, bboxEnt->s.origin2);
		...
		G_RailTrail( ent->client->ps.clientNum, b1, b2, colorRed, 10 );
	...
	if (
		...
		if (							// codePRK g_minHits
			ent->client->sess.hits >= 20 && //g_minHits.integer &&
			teamHitPct > g_teamDamageRestriction.integer
			&& !( ent->r.svFlags & SVF_BOT )
		) {
		
			if (
				g_kickTime.integer > 0
			) {
