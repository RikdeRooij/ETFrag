...
//	G_SetOrigin( ent, trace->endpos );
	...
	if( ((ent->s.weapon == WP_PANZERFAUST) && (g_panzerwar.integer > 1)) ) {
		event = EV_MISSILE_HIT;
	}
...
// ------------------------------------------------------
/*				// codePRK
			} else if( (ent->s.weapon == WP_GPG40 ||
					ent->s.weapon == WP_M7)
					&& ( ent->count ) ) {
				
				// need this now
				trap_Trace( &tr, ent->r.currentOrigin, 0, 0, origin, ent->r.ownerNum, ent->clipmask );

				if(  tr.fraction != 1 ) {
// --
					trace_t		tr2;
					vec3_t oldPos, newPos;

					// trace back for sky
					VectorCopy(origin, oldPos);
					VectorSubtract(  ent->r.currentOrigin, origin, newPos );
					VectorAdd( origin, newPos, newPos );
					trap_Trace(&tr2, oldPos, 0, 0, newPos,  ent->r.ownerNum, MASK_ALL );
					G_RailTrail( oldPos, newPos, colorMagenta, -1 );
					...
					// that failed, lets try this 2
					else {
						VectorCopy(origin, oldPos);
						VectorCopy(origin, newPos);
						newPos[2] += 65536; // 65536
						trap_Trace(&tr2, oldPos, 0, 0, newPos,  ent->r.ownerNum, MASK_ALL );
...
// ------------------------------------------------------
		// codePRK
		if( (ent->s.weapon == WP_GPG40 ||
				ent->s.weapon == WP_M7) ) {

			if ( tr.surfaceFlags & SURF_SKY ) { //( tr.surfaceFlags & SURF_NOIMPACT )
				// goes through sky
				ent->count = 1;
...
gentity_t *fire_rocket (gentity_t *self, vec3_t start, vec3_t dir) {
	...
	bolt->splashRadius = 350; //G_GetWeaponDamage(WP_PANZERFAUST);	// Arnout : hardcoded bleh hack
